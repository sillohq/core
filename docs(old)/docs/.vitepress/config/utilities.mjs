...
export function generateDynamicOGImage(pageData) {
  ...
  const logo = 'https://sillo-labs.github.io/sillo/logo.png'
  const website = 'sillo-labs.github.io'
  const name = 'sillo Labs'

  // Extract title and description from page data
  let title = pageData?.title || pageData?.frontmatter?.title || 'sillo'
  ...
  if (title === 'sillo' && pageData?.content) {
...
export const transformHead = ({ pageData }) => {
  ...
  const pageTitle = pageData?.title ? `${pageData.title} | sillo` : 'sillo - Python Web Framework'
  const pageDescription = pageData?.description || 'sillo is a modern, fast, and secure web framework for Python. It is designed to be easy to use and understand, while also being powerful and flexible.'
  ...
  return [
    ...
    ['meta', { property: 'og:image:alt', content: `${pageData?.title || 'sillo'} - Python Web Framework` }],

    // Twitter Card tags
    ['meta', { name: 'twitter:card', content: 'summary_large_image' }],
    ['meta', { name: 'twitter:description', content: pageDescription }],
    ['meta', { name: 'twitter:image', content: ogImageUrl }],
    ['meta', { name: 'twitter:image:alt', content: `${pageData?.title || 'sillo'} - Python Web Framework` }],
