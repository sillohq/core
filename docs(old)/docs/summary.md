# sillo Comprehensive Guide

This guide provides an in-depth exploration of the sillo framework, combining core concepts, advanced features, and best practices into a single resource.

## Getting Started

sillo is designed to be the fastest and most developer-friendly async Python web framework.
...
sillo provides a powerful routing engine that supports path parameters, type validation, and nested routers.

### Basic & Parameterized Routing

You can define routes with typed parameters. sillo automatically validates and converts these parameters.
...
sillo makes it easy to access body data, forms, and files.
...
sillo has a built-in authentication system with support for custom backends and fine-grained permissions.
...
from sillo.auth.backends import JWTBackend
...
sillo prioritizes security with a suite of built-in protections conveniently bundled in `SecurityMiddleware`.
...
For deeper integrations, sillo provides a `Context` object that allows you to access application-level or request-level data within your dependencies.

```python
from sillo import Context
...
sillo provides a powerful templating system built on top of Jinja2.
...
sillo automatically generates OpenAPI specifications and interactive documentation (Swagger UI / ReDoc).
