...
sillo is a modern, async-first Python web framework built on ASGI. It combines high performance, developer-friendly features, and a clean, maintainable architecture. This page introduces the fundamental concepts that make sillo powerful and easy to use.

## Framework Philosophy

sillo is designed around several core principles:

### 1. **Simplicity First**

sillo prioritizes simplicity in its API. Common tasks are straightforward, while advanced features remain accessible.
...
Every design decision in sillo considers performance. The framework is optimized for high-throughput applications without sacrificing developer experience.
...
ASGI (Asynchronous Server Gateway Interface) is the foundation that enables sillo to handle concurrent connections efficiently.
...
sillo uses a layered architecture:
...
```python
from sillo.middleware import CORSMiddleware, SecurityMiddleware
...
Handlers process requests and return responses. sillo automatically serializes responses.
...
sillo supports dependency injection for clean, testable code. Use `Query`, `Header`, and `Cookie` parameter extractors for request data.

```python
from sillo import Depend, Query, Header
...
sillo includes built-in security features:
...
sillo makes testing easy with built-in tools.

```python
import pytest
from sillo.testing import Client
