...
The OpenAPI Builder in sillo automatically generates comprehensive OpenAPI 3.0 documentation for your API, providing interactive documentation through Swagger UI and ReDoc interfaces.
...
app = silloApp(config=MakeConfig({"openapi": config}))

# Alternative: Using MakeConfig with Pydantic models directly
app = silloApp(config=MakeConfig({
    "openapi": {
        "title": "My API",
        "version": "2.0.0", 
        "description": "A comprehensive API for managing users and resources",
...
```python
from sillo.openapi.models import HTTPBearer, APIKey, OAuth2, OAuthFlows, OAuthFlowAuthorizationCode
...
```python
from sillo.openapi.models import Path, Schema
...
```python
from sillo.openapi.models import Header, Schema
...
```python
from sillo.openapi.models import Schema
...
- [Application](./sillo-app.md) - Application setup
