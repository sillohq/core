...
The `silloApp` class is the core application class in sillo, serving as the main entry point for building web applications.
...
```python
app = silloApp(description="A powerful REST API built with sillo")
```
...
app = silloApp(server_error_handler=custom_error_handler)
```

### lifespan: Optional[lifespan_manager]
**Type**: `Callable[[silloApp], AsyncContextManager[bool]]`  
**Default**: `None`
...
app = silloApp(dependencies=[Depend(get_db)])
```
...
The `silloApp` class implements the ASGI protocol and can be used with any ASGI server.
