...
The WebSocket system in sillo provides real-time, bidirectional communication between clients and servers with full async/await support and comprehensive error handling.
...
```python
from sillo.websockets.errors import WebSocketDisconnect
...
sillo provides a pluggable history management system for WebSocket channels, allowing you to customize how message history is stored and retrieved. By default, messages are stored in memory, but you can implement custom backends like Redis, databases, or file systems.
...
```python
from sillo.websockets import NoOpHistoryManager, ChannelBox
...
```python
from sillo.websockets import InMemoryHistoryManager, ChannelBox
...
from sillo.websockets import Channel, ChannelBox, FileHistoryManager
