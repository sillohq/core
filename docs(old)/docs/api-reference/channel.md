...
The `Channel` class represents a WebSocket connection in sillo, providing methods for sending messages and managing connection lifecycle. It's used internally by the WebSocket system to handle individual client connections.
...
```python
from sillo.websockets import WebSocket, Channel
