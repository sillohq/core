...
The `Group` class in sillo provides a way to organize and mount sub-applications or collections of routes under a common path prefix. It acts as a mounting point that can contain either an ASGI application or a collection of routes, with support for middleware and path parameter handling.
...
An ASGI application to mount at the specified path. This can be any ASGI-compatible application, including other sillo apps, FastAPI apps, or custom ASGI applications.
...
```python
from sillo import silloApp
from sillo.routing import Router, Route, Group
...
- [silloApp](sillo-app.md) - Main application class