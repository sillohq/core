...
#  Robust Dependency Injection in sillo

sillo provides a modern, flexible, and powerful dependency injection (DI) system inspired by the best of FastAPI and Starlette, but with its own unique features. This guide will help you master DI in sillo, from simple use cases to advanced patterns.

---

##  What is Dependency Injection?

Dependency Injection is a design pattern that allows you to declare external resources, services, or logic ("dependencies") that your route handlers or other dependencies need. sillo will automatically resolve and inject these dependencies for you.
...
For resources that need cleanup (e.g., DB sessions), use generator or async generator dependencies. sillo will handle cleanup automatically:
...
sillo supports dependencies that apply to all routes in the app or in a router. This is useful for things like authentication, database sessions, or any logic you want to share across multiple routes.

- **App-level dependencies**: Set with the `dependencies` argument on `silloApp`. These run for every request to the app.
...
You can also use `context=Context()` as a parameter, and sillo will inject the current context automatically.

---

##  Deep Context Propagation

sillo supports context propagation through deeply nested dependencies:
