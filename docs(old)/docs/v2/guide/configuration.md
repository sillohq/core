...
sillo provides a simple way to manage configuration settings, using the `MakeConfig` class.
...
You can access the configuration using the `config` attribute of the `silloApp` instance:
...
```python
from sillo.config import get_config, set_config
from sillo import silloApp
