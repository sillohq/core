...
description: Welcome to sillo! This comprehensive guide will walk you through everything you need to know to get started with sillo, a modern, async-first Python web framework designed for high-performance applications.
head:
  - - meta
    - property: og:title
      content: Getting Started with sillo
  - - meta
    - property: og:description
      content: Welcome to sillo! This comprehensive guide will walk you through everything you need to know to get started with sillo, a modern, async-first Python web framework designed for high-performance applications.
---

::: danger ⚠️ You're viewing deprecated documentation
**This is sillo v2 documentation, which is no longer maintained.** 
...
sillo is a cutting-edge Python web framework that combines the best of modern web development practices with exceptional performance. Built on ASGI (Asynchronous Server Gateway Interface), sillo provides a clean, intuitive API that makes building scalable web applications straightforward and enjoyable.
...
sillo stands out from other Python web frameworks for several reasons:

**Performance**: Unlike traditional WSGI frameworks, sillo leverages ASGI to handle thousands of concurrent connections efficiently. This means your application can serve more users with fewer resources.
...
**Modern Python**: sillo fully embraces modern Python features like type hints, async/await, and dataclasses, making your code more maintainable and less error-prone.
...
Before you begin with sillo, ensure you have the following:

### System Requirements

- **Python 3.9 or higher** - sillo requires modern Python features
...
sillo uses async/await extensively for handling concurrent requests efficiently. Don't worry if this is new to you - we'll cover it in detail throughout the documentation.


## Installation

sillo can be installed using any Python package manager. We recommend using `uv` for the fastest and most reliable experience.
...
# Create a virtual environment and install sillo
uv venv
...
# Install uv and sillo in one command
...
## Your First sillo Application

Now that you have sillo installed, let's create your first application. We'll start with a simple example and gradually build up to more complex features.
...
# If you forget to use async def for your handler, sillo will run the handler in a threadpool
...
You can run your sillo application in several ways:
...
sillo provide an intaractive  API documentation interface. This allows you to test your API endpoints and explore the available routes.
...
- `silloApp()` creates a new web application instance
...
- The function must be `async` - this is a strict requirement in sillo
- `request` and `response` are automatically provided by sillo
...
Congratulations! You've successfully created and run your first sillo application. Here's what you can explore next:
...
| [Core Concepts](/guide/concepts)                                | Dive deeper into sillo architecture and concepts:<br>- [Framework Architecture](/guide/concepts)<br>- [Async Python](/guide/async-python)<br>- [Configuration](/guide/configuration)                                            |
...
sillo is built on ASGI, which requires async handlers for optimal performance. Async handlers allow sillo to handle multiple requests concurrently without blocking, leading to better performance and scalability.
...
If a required parameter is missing or invalid, sillo will return a 422 error:
...
### How does sillo compare to FastAPI/Django/Flask?

sillo offers a unique combination of simplicity and performance:
...
Remember, everyone starts somewhere! Don't hesitate to ask questions and experiment with the code. The sillo community is here to help you succeed.

---

Now that you have the basics down, let's explore more advanced features. Start with the [Routing Guide](/guide/routing) to learn how to create more complex applications, or jump into [Core Concepts](/guide/concepts) to understand how sillo works under the hood.
