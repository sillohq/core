...
async def fetch_posts(user_id: int):
    return [{"title": "sillo vs FastAPI"}, {"title": "Async Python"}]
...
**sillo** is a younger, lightweight ASGI framework inspired by Express.js but tailored for Python’s async world . Unlike FastAPI, which emphasizes schema validation and documentation, sillo focuses on**speed , simplicity , and extensibility **. It aims to be the minimal but powerful foundation for APIs, with explicit coding style and no hidden “magic.”
...
sillo adopts a similar layered approach but with **greater emphasis on control** 🎛️. Middleware wraps around requests in an “onion-like” pattern :
...
* In **sillo**, a `Context`-like pattern emerges via the `Request` and `Response` objects. All state, headers, and attributes related to a request lifecycle are available there. Developers can also extend these objects to add their own utilities — similar to how Egg extended Koa’s `Context`.
...
* **sillo** allows writing middleware that catches and handles errors in a simple async flow:
...
sillo, being minimal, introduces **backends and extensions** :
...
* **sillo**: Still evolving, but its vision is clear — a framework that is:
...
* sillo = lean, fast, explicit, extensible .

Both embrace Python’s async/await world , but their philosophies diverge: FastAPI optimizes for developer convenience with validation and docs, while sillo optimizes for speed, simplicity, and extensibility.
