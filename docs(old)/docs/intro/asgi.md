...
```txt
 ...
 | (Client)   |       | (Uvicorn, etc) |       | (sillo, FastAPI) |
 +------------+       +----------------+       +-------------------+
                          |          |
                          |          |
                +---------+          +-------------+
...
- Your **ASGI app** (like**sillo**) just needs to handle the `scope`, `receive`, and `send` interfaces.
- ASGI enables **streaming responses, background jobs,** and**real-time features** easily.

---

## Why ASGI Matters for sillo

sillo is an **ASGI-native** framework. This means:
