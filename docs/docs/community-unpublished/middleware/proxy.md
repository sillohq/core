...
description: A production-ready proxy middleware for the sillo ASGI framework.
---

#  Proxy Middleware

A production-ready proxy middleware for the sillo ASGI framework that handles applications running behind proxy servers, load balancers, and CDNs.
...
```python
from sillo_contrib.proxy import (
...
```python
from sillo_contrib.proxy import is_trusted_proxy
...
```python
from sillo_contrib.proxy import ProxyMiddleware
