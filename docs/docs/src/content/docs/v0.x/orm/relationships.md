...
one [migrations](/v0.x/orm/migrations/#the-app-label) use. A string rather than the
...
Better still, do not delete, [soft delete](/v0.x/orm/mixins/#softdeletesmixin), and
...
[`CascadesDeletesMixin`](/v0.x/orm/mixins/#cascadesdeletesmixin), which only applies
...
published posts appears three times. [`distinct()`](/v0.x/orm/values/#distinct) is
what collapses them.
...
three, a recursive CTE in [raw SQL](/v0.x/orm/raw-sql/) is the right tool, or store
...
across a many-to-many. Check with [`explain()`](/v0.x/orm/queries/#explain) when a
query gets deep.
...
- [Eager loading](/v0.x/orm/eager-loading/): `select_related`, `prefetch_related`
  and the N+1
- [Lookups](/v0.x/orm/lookups/): everything you can put after `__`
- [Field reference](/v0.x/orm/field-reference/): the non-relational types
