...
Wrap the write in a [transaction](/v0.x/orm/transactions/) if the two must be
atomic, or (usually better) dispatch a [queued job](/v0.x/guides/work/queue/) so a
...
| Instead of | Use |
| --- | --- |
| Validating in `before_save` | [`ValidatesBeforeSaveMixin`](/v0.x/orm/mixins/#validatesbeforesavemixin): same timing, clearer name |
| Cascading deletes in `before_delete` | A database `on_delete=CASCADE` |
| Cross-aggregate work in `after_save` | An [application event](/v0.x/guides/events/) or a [job](/v0.x/guides/work/jobs/) |
