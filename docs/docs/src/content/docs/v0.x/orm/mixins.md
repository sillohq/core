...
serialisation) are already on [the base model](/v0.x/orm/models/); the mixins exist
...
To flip it, add a [global scope](/v0.x/orm/scopes/#global-scopes):
...
The fields themselves are on [the base model](/v0.x/orm/models/#what-you-get-for-free);
...
model](/v0.x/orm/pydantic/). See the caution under
[Serialisation](/v0.x/orm/models/#serialisation).
...
validation belongs [in front of the handler](/v0.x/guides/validation/), where it can
...
[events](/v0.x/orm/events/) per child.
:::

Wrap it in a [transaction](/v0.x/orm/transactions/). A cascade that fails halfway
