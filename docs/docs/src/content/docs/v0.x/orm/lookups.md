...
[relation to traverse](/v0.x/orm/relationships/#spanning-relations-in-queries). With
...
[search box](/v0.x/orm/admin-customising/#search) is `icontains`, and is meant for
...
[`.sql()`](/v0.x/orm/queryset/#inspecting) before relying on it.
...
[`JSONField`](/v0.x/orm/field-reference/#json) has its own set, not the ones above:
...
appears twice. [`distinct()`](/v0.x/orm/values/#distinct) collapses them.
...
what lets it see the aggregate. See [Aggregation](/v0.x/orm/aggregation/).
...
- [The QuerySet API](/v0.x/orm/queryset/)
- [Relationships](/v0.x/orm/relationships/)
