...
The [admin panel](/v0.x/orm/admin/) uses it as a row's default label, and so does
...
Prefer a [Pydantic response model](/v0.x/orm/pydantic/), which states the shape once
...
use [`upsert`](/v0.x/orm/bulk/#upsert), which is one statement.
:::
...
for. To make it automatic, add a [global scope](/v0.x/orm/scopes/#global-scopes).
:::

More in [Mixins](/v0.x/orm/mixins/#softdeletesmixin).
...
[global scopes](/v0.x/orm/scopes/). If you set your own `manager`, subclass
...
files](/v0.x/start/personalisation/#model-files) when it renames a project.
:::

## The rest

- [Fields](/v0.x/orm/fields/): the field types Record adds
- [Mass assignment](/v0.x/orm/mass-assignment/): `fillable` and `guarded`
- [Mixins](/v0.x/orm/mixins/): the composable behaviours
