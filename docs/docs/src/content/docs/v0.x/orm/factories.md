...
`create_many` saves one at a time, so [model events](/v0.x/orm/events/) and
[validation](/v0.x/orm/mixins/#validatesbeforesavemixin) all run. For hundreds of
rows where you do not need either, [`bulk_create`](/v0.x/orm/bulk/) is much faster.
...
- [Seeding and fixtures](/v0.x/orm/seeding/): for populating a real database.
- [Test client](/v0.x/guides/start/testing/): the HTTP side of a test.
