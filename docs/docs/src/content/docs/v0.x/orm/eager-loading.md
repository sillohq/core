...
- [`ForeignKeyField`](/v0.x/orm/relationships/#foreign-keys)
- [`OneToOneField`](/v0.x/orm/relationships/#one-to-one)
...
- [many-to-many](/v0.x/orm/relationships/#many-to-many) (`post.tags`)
...
**The plan.** [`explain()`](/v0.x/orm/queries/#explain) for one query at a time.

**In the admin.** A `list_display` naming a relation is one query per row.
Fix it in [`get_queryset`](/v0.x/orm/admin-registering/#get_queryset):
...
The honest check is [`explain()`](/v0.x/orm/queries/#explain) and the query count,
not intuition.

## Projections instead

Sometimes the relation is wanted for one column, and a
[projection](/v0.x/orm/values/) is cheaper than loading the object at all:
