...
```python
@app.patch("/me")
async def update_me(request, response):
    await request.user.update_from_dict(await request.json())
    return response.json(request.user.to_dict())
```
...
[admin access](/v0.x/orm/admin-permissions/).
...
@app.patch("/me")
async def update_me(request, response, payload: ProfileUpdate):
    await request.user.update_from_dict(payload.model_dump(exclude_unset=True))
    return response.json(request.user.to_dict())
```

Now the shape is declared once, enforced before the handler runs, and
[documented in OpenAPI](/v0.x/guides/validation/openapi/) for free. `fillable`
...
- [Models](/v0.x/orm/models/): the base class.
- [Validation](/v0.x/guides/validation/): the layer this should sit behind.
