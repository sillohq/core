...
[Pagination](/v0.x/orm/pagination/#ordering-is-not-optional).
...
[`explain()`](/v0.x/orm/queries/#explain).
...
what makes [`get_or_create`](/v0.x/orm/models/#fetch-shortcuts) and
[`upsert`](/v0.x/orm/bulk/#upsert) safe under concurrency, without it, two
...
- **Low-cardinality columns.** An index on a boolean rarely helps; the database
  will scan anyway.
- **`LIKE '%term%'`**, which is what [`icontains`](/v0.x/orm/lookups/) compiles to.
  A leading wildcard cannot use a B-tree index. For real search, use a trigram
  index on PostgreSQL or a search service.

Verify with [`explain()`](/v0.x/orm/queries/#explain) rather than assuming.
...
[`ValidatesBeforeSaveMixin`](/v0.x/orm/mixins/#validatesbeforesavemixin), which only
...
This is how [`sillo.record.Model`](/v0.x/orm/models/) itself supplies `created_at`,
...
files](/v0.x/start/personalisation/#model-files) when it renames a project. Setting
...
scopes](/v0.x/orm/scopes/#global-scopes). Replacing it with a manager that does not
...
a second set of [migration commands](/v0.x/cli/standalone-consoles/#record_commands).
...
[admin](/v0.x/orm/admin/) builds its forms from `fields_map`, and
[`mass_assignable_fields`](/v0.x/orm/mass-assignment/) reads `fields`. Useful for
