...
[`Subquery`](/v0.x/orm/filtering/#subquery) is better still, since it never brings
...
- no `to_dict()`, no [casts](/v0.x/orm/casting/), no model methods;
- no `save()`: you cannot write these back;
- no [property or computed attributes](/v0.x/orm/models/).

A [cast](/v0.x/orm/casting/) field fetched with `values()` gives you the **encoded**
...
[many-to-many](/v0.x/orm/relationships/#filtering-across-it) produces one row per
...
function in [raw SQL](/v0.x/orm/raw-sql/), or a
[`Subquery`](/v0.x/orm/filtering/#subquery).
...
See [Aggregation](/v0.x/orm/aggregation/) for the fuller treatment.
