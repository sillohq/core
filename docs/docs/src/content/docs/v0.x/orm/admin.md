...
| `auth_backend` | A full [auth backend](/v0.x/orm/admin-permissions/#authentication-backends). Overrides `user_model`. |

Returns the `AdminSite`, which is what you register models on.

### It needs a database

The admin reads and writes through the ORM, so [`setup_record`](/v0.x/orm/setup/)
...
Any [`UserBaseModel`](/v0.x/guides/users/) subclass works.
...
not enough. See [Permissions](/v0.x/orm/admin-permissions/).
...
([Permissions](/v0.x/orm/admin-permissions/)), and consider disabling the admin
...
- [Registering models](/v0.x/orm/admin-registering/): `ModelAdmin` and what it
  controls.
- [Customising](/v0.x/orm/admin-customising/): lists, search, filters, forms.
