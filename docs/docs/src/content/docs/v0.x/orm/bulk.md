...
Returns the instances. [Casts](/v0.x/orm/casting/) are applied. Each instance is
...
[`without_global_scopes()`](/v0.x/orm/scopes/#escaping-them), so upserting a
...
[`get_or_create`](/v0.x/orm/models/#fetch-shortcuts) is a `SELECT` then an `INSERT`,
...
| | Applied? |
| --- | --- |
| [Casts](/v0.x/orm/casting/) | Yes |
| [Model events](/v0.x/orm/events/) | **No** |
| [`ValidatesBeforeSaveMixin`](/v0.x/orm/mixins/#validatesbeforesavemixin) | **No** |
...
batches. A [transaction](/v0.x/orm/transactions/) makes the whole thing one unit.
