...
The alternative is [`get_or_none()`](/v0.x/orm/models/#fetch-shortcuts) and an
...
validation](/v0.x/guides/validation/) runs before the handler and produces
...
same signature as any other [exception handler](/v0.x/guides/error-handling/), so
...
- **Deadlocks and serialisation failures.** These arrive as `OperationalError`
  ...
  [retry](/v0.x/orm/transactions/#concurrency).
