...
Plus the [global flags](/v0.x/start/install/#global-flags): `-v`, `-q`, `-h`.
...
[Custom starters](/v0.x/start/custom-starters/#pinning-a-revision).
...
[package manager](/v0.x/start/package-managers/) is available:
...
1. **Validate the name.** Before anything is fetched: it becomes both a
   ...
   See [Project names](/v0.x/start/naming/).
...
5. **Personalise**: rewrite the name into the files that carry it, and create
   `.env` with fresh secrets. See [Personalisation](/v0.x/start/personalisation/).
...
See [Errors and exit codes](/v0.x/start/errors/).
