...
document](/v0.x/pydantic/openapi/) describe the request without introspecting the
...
[422 with every failure listed](/v0.x/pydantic/errors/):
...
[Configuration](/v0.x/pydantic/config/#defaults-worth-adopting).

## What the model must not do

A request model validates **shape**. It cannot validate facts about your
database, because [Pydantic validators are synchronous](/v0.x/pydantic/validators/#where-validation-belongs):
...
which the [exception handlers](/v0.x/orm/exceptions/) already turn into a 409. That
...
The same reasoning as [mass assignment](/v0.x/orm/mass-assignment/) on the ORM side,
...
to make it optional. See [Types](/v0.x/pydantic/types/#unions-and-optionals).
...
See [Parameters](/v0.x/pydantic/parameters/#form-and-file).
...
Both go straight into the [OpenAPI document](/v0.x/pydantic/openapi/). A single
