...
a `PostOut` model states the shape once. See [Patterns](/v0.x/pydantic/patterns/).
...
form, when you have set one. See [Fields](/v0.x/pydantic/fields/#aliases).

Sillo's [response models](/v0.x/pydantic/response-models/) do not set this for you,
so a model with aliases needs it explicitly, or an
[alias generator](/v0.x/pydantic/config/) plus `serialize_by_alias`.
...
[`SecretStr`](/v0.x/pydantic/types/#pydantics-own-types) keeps the value out of
...
async def get_post(request, response, id=Path(type=int)):
    post = await Post.get(id=id)
    return response.json(PostOut.model_validate(post).model_dump(mode="json"))
```

With `response_model=` declared, Sillo validates and shapes the return value
for you. See [Response models](/v0.x/pydantic/response-models/), which is the
