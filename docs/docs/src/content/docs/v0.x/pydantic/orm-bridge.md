...
| | [Record model](/v0.x/orm/models/) | Pydantic model |
| --- | --- | --- |
...
With [`response_model=`](/v0.x/pydantic/response-models/) declared, Sillo does the
...
at [Pydantic schemas](/v0.x/orm/pydantic/) on the ORM side.
...
[that page](/v0.x/orm/pydantic/#the-type-mapping):
...
Same reasoning as [`fillable` over `guarded`](/v0.x/orm/mass-assignment/#which-to-reach-for)
on the write side.
...
[`fillable`](/v0.x/orm/mass-assignment/) on the model is the second line of defence
behind both.
...
One enum, used by [`CharEnumField`](/v0.x/orm/field-reference/#enums) and by the
