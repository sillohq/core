...
`minimum: 1` in your [OpenAPI document](/v0.x/pydantic/openapi/), so the validation
...
`strict` is for a [JSON body](/v0.x/pydantic/request-models/), where the client
controls the types.
:::
...
[error paths](/v0.x/pydantic/errors/#loc); `page_size` is the Python name.
...
filename as a path. See [File uploads](/v0.x/guides/file-upload/).
:::

Do not mix `Form`/`File` with [`request_model`](/v0.x/pydantic/request-models/). A
...
string from a malformed body. See [Validation errors](/v0.x/pydantic/errors/).
...
- [Request models](/v0.x/pydantic/request-models/): the JSON body
- [Fields](/v0.x/pydantic/fields/): the same constraints on model fields
- [Request parameters](/v0.x/guides/request-parameters/): the wider guide
