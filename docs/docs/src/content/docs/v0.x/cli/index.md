...
[`setup_record`](/v0.x/guides/record/) put on `app.state` is the one migrations run
against; the user model the application authenticates against is the one
`user:create` writes to; the scheduler
[`setup_scheduler`](/v0.x/guides/work/scheduler/) registered is the one
`schedule:list` reads.
...
[`sillo-start`](/v0.x/start/), a separate tool, so the framework does not have to
carry a copy of a starter it would then have to keep in step with.

## Where to go next

- [Finding your application](/v0.x/cli/discovery/): the three places `sillo` looks,
  and what to do when it looks in the wrong one.
- [Framework commands](/v0.x/cli/framework-commands/): `version` and `routes`.
- [Database commands](/v0.x/cli/database/): the migration workflow end to end.
- [User commands](/v0.x/cli/users/): creating and managing accounts.
- [Queue](/v0.x/cli/queues/) and [scheduler](/v0.x/cli/scheduler/) commands.
- [Writing your own commands](/v0.x/cli/custom-commands/): the part most projects
  reach within a week.
