...
1. Parses all four headers → stores on `request.state.accepts` (full dict) and
   `request.state.accepts_parsed` (pre-parsed `AcceptItem` lists)
...
async def get_data(request: Request, response: Response):
    ...
    if content_type == "text/csv":
        return response.text(to_csv(data))
...
```python
request.negotiated_content_type = best_type
request.negotiated_language = best_language
```
...
- `Content-Length: 0` replaces the original body length
...
if request.content:
    parts.append(sha256(request.content).hexdigest()[:16])
...
class MiddlewareChain:
    async def run(self, request, final_send):
...
**Response processing** ensures the header survives downstream modifications:
...
```python
# Enable caching
response.enable_caching(max_age=3600, private=True)
