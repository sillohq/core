...
[Validation](/v0.x/guides/validation/) section.
...
`Depend(get_request=True)` injects the raw `Request` into a dependency,
leaving the extraction to you:

```python
from sillo import Depend

def get_user_data(request = Depend(get_request=True)):
