...
The [Introduction](/v0.x/guides/introduction/) makes a claim in one paragraph
...
[Extending Sillo](/v0.x/advanced/extending/) and
[Auth Backends](/v0.x/advanced/auth-backends/) for the seams.

###  Convenience without mystery

Sillo will do things for you. It will not do things you cannot see.

`response.json({...})` builds and sends a JSON response. That is convenience.
It is also one function, documented, that you can call yourself or replace —
not a metaclass reaching into your module at import time. `Depend(...)` resolves your
...
Two versions of these docs exist side by side for exactly this reason. This
manual, v0.x, is the released one and the one an unversioned link lands on; the
v1.0 manual describes work in progress and says so at the top of every page.
Where 1.0 moves something out of the framework — the admin, the HTML templating
layer, WebSocket rooms — the page it moves to already exists and says where it
came from, before the release rather than after it.
...
[Advanced manual](/v0.x/advanced/) documents the internals as internals — not
...
[`sillo.config.Config`](/v0.x/guides/configuration/) — a Pydantic model, so
...
ready. See [Startup & Shutdown](/v0.x/guides/startups-and-shutdowns/).
...
[Creating a Project](/v0.x/guides/start/).
...
backend are [extras](/v0.x/guides/ecosystem/), because keeping something in
...
rate limiting, notably — the [decision is written down](/v0.x/advanced/decisions/)
with its reasoning.
...
[Concurrency](/v0.x/guides/concurrency/) explains when to use it, but this is
...
- [Introduction](/v0.x/guides/introduction/) — what the framework is, and the
  comparison table
- [Frequently Asked Questions](/v0.x/guides/faq/) — the version, dependency and
  adoption questions
- [What's in the Box](/v0.x/guides/ecosystem/) — where the line between core,
  extras and packages actually falls
- [Architectural Decisions](/v0.x/advanced/decisions/) — the same reasoning
