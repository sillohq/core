...
Long, patient retries belong in [Background Tasks](/v0.x/guides/work/background/) and the [Job Queue](/v0.x/guides/work/queue/), where nobody is waiting on the socket. If an operation can afford a minute of retries, it can afford to be a job.
...
- [Background Tasks](/v0.x/guides/work/background/): where patient retries belong
- [Job Queue](/v0.x/guides/work/queue/): durable retries that survive a restart
- [HTTP Client](/v0.x/guides/http/client/): has its own retry configuration
- [Error Handling](/v0.x/guides/error-handling/): turning exhausted retries into
  responses
- [Async Helpers](/v0.x/guides/helpers/async/): sync and async detection, which the
  decorator uses
