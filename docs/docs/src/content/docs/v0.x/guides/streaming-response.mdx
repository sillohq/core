...
```python
from sillo import Request, Response
...
```python
return response.sse(source(), keepalive=30.0)   # or None to disable
```
...
async def events(request: Request, response: Response):
    ...
    return response.sse(source(), retry=5000)
```
...
multiplayer. See [WebSockets](/v0.x/guides/websockets/).
