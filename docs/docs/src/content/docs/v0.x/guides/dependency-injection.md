...
(For a route-level `request_model`, you can also bind the validated model to a parameter by matching its name to the model; see [Request Parameters](/v0.x/guides/request-parameters/) and [Handling Inputs](/v0.x/guides/request-inputs/).)
...
[Installation](/v0.x/guides/installation/) for setup). The whole DI tree resolves
...
- [Request Parameters](/v0.x/guides/request-parameters/): `Query`, `Header`,
  `Cookie` extractors in handlers and dependencies
- [Handlers](/v0.x/guides/handlers/): the handler contract and return values
- [Routers & Sub-Apps](/v0.x/guides/routers-and-subapps/): organize
  dependency-protected function handlers behind a prefix
- [Middleware](/v0.x/guides/middleware/): request-scoped logic that runs for every
  request, not just injected ones

##  Related topics

- [Routing](/v0.x/guides/routing/): path syntax, `name=`, and route options like
  `request_model`
- [Error Handling](/v0.x/guides/error-handling/): turning validation failures into
  clean responses
- [Authentication](/v0.x/guides/authentication/): `useAuth` and the auth dependency
...
pointless. Build it once in a [startup hook](/v0.x/guides/startups-and-shutdowns/)
...
matters. [Middleware](/v0.x/guides/middleware/) applies by position in the stack
