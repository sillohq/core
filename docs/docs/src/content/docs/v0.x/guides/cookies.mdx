...
async def set_cookie(request, response):
    return response.json({"status": "Cookie set"}).set_cookie(
        key="user_id",
        value="12345",
...
async def remember_me(request, response):
    return response.text("Cookie set for 10 years").set_permanent_cookie(
        key="remember_me",
        value="true"
    )
```
...
async def multi_cookie(request, response):
    ...
    return response.json({"status": "Cookies set"}).set_cookies(cookies)
```
...
async def login(request, response):
    ...
    if validate_credentials(data["username"], data["password"]):
        return response.json({"status": "Login successful"}).set_cookie(
            key="auth_token",
...
async def protected_route(request, response):
    ...
    if not user:
        return response.status(401).json({"error": "Invalid token"}).delete_cookie("auth_token")
...
- [Sessions](/v0.x/guides/sessions/): what most cookies carry
- [Session Auth](/v0.x/guides/session-auth/): the authentication flow
- [CSRF](/v0.x/guides/csrf/): why cookies need `SameSite` and a token
- [Headers](/v0.x/guides/headers/): `Set-Cookie` and header size limits
...
on `max_age` in [Crypto helpers](/v0.x/guides/helpers/crypto/), so if the value
