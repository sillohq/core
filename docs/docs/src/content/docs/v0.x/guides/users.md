...
See the [Permissions](/v0.x/guides/permissions/) guide for the full API: groups,
...
Each mixin reads `int(str(self.identity))` as the `user_id`, which matches how `SessionAuthBackend` and `User.load_user` resolve identities. See [JWT](/v0.x/guides/jwt-auth/), [Sessions](/v0.x/guides/session-auth/), and [API Keys](/v0.x/guides/api-keys/) for the methods these add.
...
- [Protecting Routes](/v0.x/guides/protecting-routes/): `useAuth` and
  `has_permission`
