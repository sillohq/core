...
- [Record: Scopes & Events](/v0.x/guides/record/scopes-events): model lifecycle
  events (`before_create`, `after_create`, …) and observers.
- [Work: Events](/v0.x/guides/work/events): the typed `@dataclass` event system used
  by the work queue.
- [WebSockets + Events](/v0.x/guides/websockets/events): bridging events to
