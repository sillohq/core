...
Returning a response object directly (e.g. `return response.json({"error": ...}, status_code=401)`) works the same way. Raising `HTTPException` is cleaner when you have an exception handler registered (see [Error Handling](/v0.x/guides/error-handling/)).
...
Prefer raising `HTTPException` and letting a registered handler format the error (see [Error Handling](/v0.x/guides/error-handling/)) over hand-rolling JSON in every middleware.
...
See [Security](/v0.x/guides/security/), [CSRF](/v0.x/guides/csrf/), [CORS](/v0.x/guides/cors/), [Request Lifecycle](/v0.x/guides/request-lifecycle/), [URL Normalization](/v0.x/guides/url-normalization/), and [Content Negotiation](/v0.x/guides/content-negotiation/) for each module's options.
...
- [Handlers](/v0.x/guides/handlers/): what middleware wraps
- [Routing](/v0.x/guides/routing/): `middleware=` on routes and routers
- [Error Handling](/v0.x/guides/error-handling/): format errors raised in middleware
- [Dependency Injection](/v0.x/guides/dependency-injection/): inject shared logic
  instead of middleware when it's per-handler
- [Request Lifecycle](/v0.x/guides/request-lifecycle/): `RequestId` is middleware
  under the hood
