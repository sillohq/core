...
```python
from sillo import Depend, Path, Query
...
```python
@app.post("/teams/{team_id}/users", request_model=UserCreate)
async def create_user(request, response, team_id, user):
...
Everything from the [parameter type catalog](/v0.x/guides/validation/parameters/#the-type-catalog) works in a model, plus the container types that only make sense in JSON:
...
See [Validation errors](/v0.x/guides/validation/errors/) for the full contract and the complete error-type catalog.
...
@app.post("/users", request_model=UserCreate)
async def create_user(request, response, user):
    raw = await request.json          # served from cache
