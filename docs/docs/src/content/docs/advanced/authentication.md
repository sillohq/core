...
```python
async def AuthErrorHandler(request, response, exc: HTTPException):
...
async def authenticate(self, request: Request) -> bool:
    ...
    # Reads from request.scope["user"] — either set by the
...
async def manage_users(request, response):
    return response.json({"status": "ok"})
```
...
async def critical(request, response):
...
async def org_members(request, response):
    ...
```
...
async def test_custom_backend():
    ...
    # Mock request with valid credential
    request = MockRequest(headers={"X-Token": "valid-token"})
    result = await backend.authenticate(request)
...
async def test_useAuth_required():
    ...
    request = MockRequest(scope={"user": UnauthenticatedUser()})
...
async def test_useAuth_optional():
    ...
    result = await gate.authenticate(request)
