...
```python
# base.py:389-448
async def resolve_validated_params(dependant, request):
    ...
    for node_id, validator in dependant._validator_plan:
        node_values, node_errors = validator.validate_sync(request)
        ...
...
```python
async def handler(request, response, name=Form(), email=Form(type=str)):
```
...
```python
async def handler(request, response, name=Form(), avatar=File()):
```
