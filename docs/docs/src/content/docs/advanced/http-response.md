...
description: "BaseResponse, JSON/File/Streaming/Redirect responses, Responder pattern"
---
...
```python
# core/sillo/core/http/response.py:1117-1119
class Responder:
...
The `Responder` is bound to a `Request` at construction time. This allows
...
Each builder creates the appropriate response subclass and stores it in
`self._response`:

```python
# JSON
def json(self, data, status_code=200, headers={}, indent=None,
         ensure_ascii=True, custom_encoder=None):
    new_response = JSONResponse(
        content=data, headers=headers, status_code=status_code,
        indent=indent, ensure_ascii=ensure_ascii, custom_encoder=custom_encoder,
    )
    self._response = new_response
    return self

# Plain text
def text(self, content, status_code=200, headers={}):
...
# HTML
def html(self, content, status_code=200, headers={}):
...
def file(self, path, filename=None, content_disposition_type="inline",
    ...
    new_response = FileResponse(
        path=path, filename=filename, status_code=status_code,
        headers=headers, content_disposition_type=content_disposition_type,
    )
    self._response = new_response
    return self
...
def stream(self, iterator, content_type="text/plain", status_code=200, headers={}):
    new_response = StreamingResponse(
        content=iterator, status_code=status_code,
...
```python
@app.get("/events")
async def sse_handler(request: Request, response: Responder):
    async def event_stream():
        while True:
            yield f"data: {await get_next_event()}\n\n"
...
def redirect(self, url=None, name=None, status_code=302, headers={}, **path_params):
    ...
    new_response = RedirectResponse(url=url, status_code=status_code, headers=headers)
    self._response = new_response
    return self
```
...
1. **Direct URL**: `response.redirect(url="/login")`
2. **Named route**: `response.redirect(name="user_detail", user_id=42)`
...
```python
def empty(self, status_code=200, headers={}):
    new_response = BaseResponse(status_code=status_code, headers=headers)
    self._response = new_response
    return self

def abort(self, status_code, detail=None, headers={}) -> typing.NoReturn:
    raise HTTPException(status_code=status_code, detail=detail, headers=headers)

def not_found(self, detail=None, headers={}) -> typing.NoReturn:
    raise NotFoundException(detail=detail, headers=headers)
```

**`abort()` and `not_found()` do NOT return.** They raise exceptions that are
...
async def get_item(request, response):
    item = await db.get(request.path_params["item_id"])
    if item is None:
        response.not_found(detail=f"Item {item_id} not found")
    return response.json(item)
```

> **Important**: The `return response.json(item)` line is **dead code** after
> `not_found()` because `not_found()` raises unconditionally. The pattern is
> "guard and short-circuit".

### 10.7 `cache()` / `no_cache()`

```python
def cache(self, max_age=3600, private=True):
...
These delegate to `BaseResponse.enable_caching()` / `disable_caching()` (§3.9).
...
return (response
    .json(data)
    .cache(max_age=3600, private=False))  # Public cache for 1 hour
```

### 10.8 `paginate()` / `apaginate()`

```python
# core/sillo/core/http/response.py:1713-1821
def paginate(self, objects, strategy="page_number",
             data_handler=SyncListDataHandler, **kwargs) -> Responder:
```
...
def set_cookie(self, key, value, max_age=None, expires=None, path="/",
               domain=None, secure=True, httponly=False, samesite="lax"):
    self._response.set_cookie(...)
    return self

def delete_cookie(self, key, path="/", domain=None):
    self._response.delete_cookie(key=key, path=path, domain=domain)
    return self

def set_permanent_cookie(self, key, value, **kwargs):
...
```python
# core/sillo/core/http/response.py — Responder class docstring example
@app.get("/users")
async def get_users(request: Request, response: Responder):
    users = await get_all_users()
...
async def debug_config(request: Request, response: Responder):
    config = await load_config()
    return response.json(config, indent=2, ensure_ascii=False)
```
...
```python
@app.get("/static-data")
async def get_static_data(request: Request, response: Responder):
...
async def download_report(request: Request, response: Responder):
...
```python
@app.get("/events")
async def event_stream(request: Request, response: Responder):
    async def generate():
        while True:
...
async def old_endpoint(request: Request, response: Responder):
    return response.redirect("/new-endpoint", status_code=301)

@app.get("/profile")
async def profile_redirect(request: Request, response: Responder):
    user = await get_current_user(request)
    return response.redirect(name="user_profile", user_id=user.id)
```
...
```python
@app.post("/login")
async def login(request: Request, response: Responder):
    data = await request.json
    user = await authenticate(data["username"], data["password"])
...
```python
@app.get("/articles")
async def list_articles(request: Request, response: Responder):
...
async def delete_user(request: Request, response: Responder):
...
### 16.10 Direct BaseResponse Usage (No Responder)
...
response.abort(403).set_header(...)  # TypeError — abort() returns NoReturn

# CORRECT: abort() raises immediately
response.abort(403, detail="Forbidden")
```
