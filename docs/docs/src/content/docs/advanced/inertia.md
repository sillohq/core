...
async def dashboard(request, response):
    return render("Dashboard", {"user": request.user.name})
```
...
```python
async def update_post(request, response):
    post = await Post.get(request.path_params["id"])
    await post.update_from_dict(await request.form())
...
| `current_request()` | `Request` | `OutsideRequestError` |
...
3. Returning a `BaseResponse` or `Responder` instead passes it through
...
async def test_concurrent_requests_isolated(self, tmp_path):
    ...
    # ... each request binds its own adapter
```
