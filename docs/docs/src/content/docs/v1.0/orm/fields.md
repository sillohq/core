...
Declared on [the base model](/v1.0/orm/models/), so you rarely write them yourself.
...
[`HasUlidMixin`](/v1.0/orm/mixins/#hasulidmixin), which needs the `python-ulid`
package:
...
For converting values without a custom column type, [casting](/v1.0/orm/casting/) is
