...
one [migrations](/v1.0/orm/migrations/#the-app-label) use. A string rather than the
...
```python
if post.author_id == ctx.user.id:      # no query
    ...

if (await post.author).id == ctx.user.id:   # a query, for the same answer
    ...
```
...
loading](/v1.0/orm/eager-loading/).
...
Better still, do not delete, [soft delete](/v1.0/orm/mixins/#softdeletesmixin), and
...
[`CascadesDeletesMixin`](/v1.0/orm/mixins/#cascadesdeletesmixin), which only applies
...
published posts appears three times. [`distinct()`](/v1.0/orm/values/#distinct) is
what collapses them.
...
three, a recursive CTE in [raw SQL](/v1.0/orm/raw-sql/) is the right tool, or store
...
across a many-to-many. Check with [`explain()`](/v1.0/orm/queries/#explain) when a
query gets deep.
...
- [Eager loading](/v1.0/orm/eager-loading/): `select_related`, `prefetch_related`
  and the N+1
- [Lookups](/v1.0/orm/lookups/): everything you can put after `__`
- [Field reference](/v1.0/orm/field-reference/): the non-relational types
