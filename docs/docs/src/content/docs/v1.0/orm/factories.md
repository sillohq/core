...
`create_many` saves one at a time, so [model events](/v1.0/orm/events/) and
[validation](/v1.0/orm/mixins/#validatesbeforesavemixin) all run. For hundreds of
rows where you do not need either, [`bulk_create`](/v1.0/orm/bulk/) is much faster.
...
- [Seeding and fixtures](/v1.0/orm/seeding/): for populating a real database.
- [Test client](/v1.0/guides/start/testing/): the HTTP side of a test.
