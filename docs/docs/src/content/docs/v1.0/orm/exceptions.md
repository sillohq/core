...
The alternative is [`get_or_none()`](/v1.0/orm/models/#fetch-shortcuts) and an
...
validation](/v1.0/guides/validation/) runs before the handler and produces
...
async def handle_conflict(ctx: HttpContext, exc):
...
same signature as any other [exception handler](/v1.0/guides/error-handling/), so
...
- **Deadlocks and serialisation failures.** These arrive as `OperationalError`
  ...
  [retry](/v1.0/orm/transactions/#concurrency).
