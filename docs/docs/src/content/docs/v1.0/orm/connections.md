...
[`setup_record`](/v1.0/orm/setup/) registers one connection named `default`.
...
`ensure_context` middleware [`setup_record` registers](/v1.0/orm/setup/#ensure_context)
...
Use [background tasks](/v1.0/guides/work/background/) or a
[queued job](/v1.0/guides/work/queue/), both of which carry it. A job also survives
...
The `prefix` on [`from_env`](/v1.0/orm/configuration/#fields) is what keeps the two
...
See [Transactions](/v1.0/orm/transactions/#multiple-connections).
...
[standalone console](/v1.0/cli/standalone-consoles/), where nothing has.
...
[migration commands](/v1.0/cli/standalone-consoles/#record_commands):
...
[`db_constraint=False`](/v1.0/orm/relationships/#db_constraintfalse) foreign key is
