...
This is the idiomatic way to make [soft deletes](/v1.0/orm/mixins/#softdeletesmixin)
the default, which the base model deliberately does not do on its own. See the
caution in [Models](/v1.0/orm/models/#soft-deletes).
...
a report over everything. The framework uses it itself. [`upsert`](/v1.0/orm/bulk/)
...
[`Model.Meta`](/v1.0/orm/models/#meta), so replacing `manager` with something that
