...
none. See [Pagination](/v1.0/orm/pagination/#ordering-is-not-optional).
...
[`fillable` over `guarded`](/v1.0/orm/mass-assignment/#which-to-reach-for).
...
A [`PasswordField`](/v1.0/orm/fields/#passwordfield) is detected and rendered as a
...
It is set-based SQL: no [events](/v1.0/orm/events/), no
[casts](/v1.0/orm/casting/), no [validation](/v1.0/orm/mixins/#validatesbeforesavemixin),
...
[`CascadesDeletesMixin`](/v1.0/orm/mixins/#cascadesdeletesmixin) still cascades.
...
Both are recorded in the [activity log](/v1.0/orm/admin/#the-activity-log) with the
