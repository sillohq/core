...
[Factories](/v1.0/orm/factories/).

## Interaction with the rest

- [`bulk_create` and friends](/v1.0/orm/bulk/) write in batches. Without a
  transaction, a failure halfway leaves earlier batches committed.
- [`after_*` model events](/v1.0/orm/events/#failure-semantics) fire inside the
  transaction when the save is inside one, so a failing hook rolls the write
  back.
- [`ensure_context`](/v1.0/orm/setup/#ensure_context) is what makes the connection
