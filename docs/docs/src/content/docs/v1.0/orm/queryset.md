...
[`Q` objects](/v1.0/orm/filtering/).

Everything after `__` is a [lookup](/v1.0/orm/lookups/) or a relation to traverse:
...
[`Meta.ordering`](/v1.0/orm/meta/#ordering) rather than adding to it.
...
handlers](/v1.0/orm/exceptions/) registered, `DoesNotExist` becomes a 404 and you
...
See [Pagination](/v1.0/orm/pagination/#counting-is-the-expensive-half).
...
- no [model events](/v1.0/orm/events/);
- no [validation](/v1.0/orm/mixins/#validatesbeforesavemixin);
- no [casts](/v1.0/orm/casting/): a cast field gets the raw Python value;
- `updated_at` does not move unless you set it;
- [`SoftDeletesMixin`](/v1.0/orm/mixins/) does not apply: `delete()` here is a real
  delete.
...
Record adds [`bulk_upsert` and `upsert`](/v1.0/orm/bulk/) on top of these.
...
See [Values and projections](/v1.0/orm/values/).
...
See [Eager loading](/v1.0/orm/eager-loading/).
...
See [Aggregation](/v1.0/orm/aggregation/).
...
Only meaningful inside a [transaction](/v1.0/orm/transactions/). Outside one the
...
See [Connections](/v1.0/orm/connections/).
...
[scopes](/v1.0/orm/scopes/) composable.
...
[`iter_all`](/v1.0/orm/queries/#iter_all), which walks in batches.
