...
serialisation) are already on [the base model](/v1.0/orm/models/); the mixins exist
...
To flip it, add a [global scope](/v1.0/orm/scopes/#global-scopes):
...
The fields themselves are on [the base model](/v1.0/orm/models/#what-you-get-for-free);
...
model](/v1.0/orm/pydantic/). See the caution under
[Serialisation](/v1.0/orm/models/#serialisation).
...
validation belongs [in front of the handler](/v1.0/guides/validation/), where it can
...
[events](/v1.0/orm/events/) per child.
:::

Wrap it in a [transaction](/v1.0/orm/transactions/). A cascade that fails halfway
