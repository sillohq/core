...
One installation wires a database into an application. The object form is the
recommended bootstrap style; the established function remains compatible.
...
`Record` and `setup_record` register all three. You do not call them.
