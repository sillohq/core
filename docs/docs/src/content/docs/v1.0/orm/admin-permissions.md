...
class PostAdmin(ModelAdmin):
    @staticmethod
    def has_delete_permission(ctx: HttpContext, obj=None):
...
class InvoiceAdmin(ModelAdmin):
    @staticmethod
    def has_add_permission(ctx: HttpContext):
        return False

    @staticmethod
    def has_change_permission(ctx: HttpContext, obj=None):
        return bool(getattr(ctx.user, "is_superuser", False))
...
@staticmethod
def has_view_permission(ctx: HttpContext):
    return bool(getattr(ctx.user, "is_superuser", False))
```
...
[Sillo's permissions](/v1.0/guides/permissions/):
...
class PostAdmin(ModelAdmin):
    @staticmethod
    def has_change_permission(ctx: HttpContext, obj=None):
        return ctx.user.has_permission("posts.edit")

    @staticmethod
    def has_delete_permission(ctx: HttpContext, obj=None):
        return ctx.user.has_permission("posts.delete")
```

Permissions have to be loaded before `has_permission` can answer: see
[Permissions](/v1.0/guides/permissions/). Loading them in middleware, once per
...
@staticmethod
def has_change_permission(ctx: HttpContext, obj=None):
    if getattr(ctx.user, "is_superuser", False):
        return True
    if obj is None:
...
[`get_queryset`](/v1.0/orm/admin-registering/#get_queryset) is what filters rows,
...
[Sillo's session middleware](/v1.0/guides/sessions/). Sign-in verifies the password
...
class SsoAuth(AuthBackend):
    ...
    async def get_user(self, ctx: HttpContext): ...
    async def login(self, ctx: HttpContext, username, password): ...
    async def logout(self, ctx: HttpContext): ...
