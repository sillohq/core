...
[`Subquery`](/v1.0/orm/filtering/#subquery) is better still, since it never brings
...
- no `to_dict()`, no [casts](/v1.0/orm/casting/), no model methods;
- no `save()`: you cannot write these back;
- no [property or computed attributes](/v1.0/orm/models/).

A [cast](/v1.0/orm/casting/) field fetched with `values()` gives you the **encoded**
...
[many-to-many](/v1.0/orm/relationships/#filtering-across-it) produces one row per
...
function in [raw SQL](/v1.0/orm/raw-sql/), or a
[`Subquery`](/v1.0/orm/filtering/#subquery).
...
See [Aggregation](/v1.0/orm/aggregation/) for the fuller treatment.
