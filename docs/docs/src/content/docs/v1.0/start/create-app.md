...
Plus the [global flags](/v1.0/start/install/#global-flags): `-v`, `-q`, `-h`.
...
[Custom starters](/v1.0/start/custom-starters/#pinning-a-revision).
...
[package manager](/v1.0/start/package-managers/) is available:
...
1. **Validate the name.** Before anything is fetched: it becomes both a
   ...
   See [Project names](/v1.0/start/naming/).
...
5. **Personalise**: rewrite the name into the files that carry it, and create
   `.env` with fresh secrets. See [Personalisation](/v1.0/start/personalisation/).
...
See [Errors and exit codes](/v1.0/start/errors/).
