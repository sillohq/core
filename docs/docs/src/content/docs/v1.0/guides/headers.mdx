...
Access incoming request headers through the `ctx.headers` property, which provides a case-insensitive dictionary-like interface:

```python {5,6}
from sillo import SilloApp, HttpContext
app = SilloApp()
@app.get("/")
async def show_headers(ctx: HttpContext):
...
sillo normalizes header names to lowercase, so `ctx.headers.get("User-Agent")` and `ctx.headers.get("user-agent")` are equivalent.
...
app = SilloApp()
@app.get("/")
async def set_headers(ctx: HttpContext):
    return (
        text("Hello, World!")
...
async def cors_middleware(ctx: HttpContext, call_next):
...
async def security_middleware(ctx: HttpContext, call_next):
    response = await call_next()
...
- `ctx.headers.get(key, default=None)` - Get a header value
- `ctx.headers.items()` - Get all headers as key-value pairs
- `ctx.headers.keys()` - Get all header names
- `ctx.headers.values()` - Get all header values
...
async def add_security_headers(ctx: HttpContext, call_next):
    response = await call_next()
...
async def add_performance_headers(ctx: HttpContext, call_next):
    response = await call_next()
    if ctx.path.endswith(('.js', '.css', '.png', '.jpg')):
...
async def login(ctx: HttpContext):
    return redirect("/dashboard").set_cookie(
        key="session_id",
        value="abc123",
        max_age=3600,
        secure=True,
...
so `ctx.headers["content-type"]` works regardless of what the client
...
it. See [Content Negotiation](/v1.0/guides/content-negotiation/).
...
- [Request Info](/v1.0/guides/request-info/): what to trust from the request side
- [Content Negotiation](/v1.0/guides/content-negotiation/): `Accept` and `Vary`
- [Cache](/v1.0/guides/cache/): server-side caching, and how HTTP caching
  complements it
- [CORS](/v1.0/guides/cors/): the cross-origin headers
- [Security](/v1.0/guides/security/): the security header set
- [Cookies](/v1.0/guides/cookies/): `Set-Cookie` and its attributes
...
accepts_json = "application/json" in ctx.headers.get("accept", "")
```
...
[Network helpers](/v1.0/guides/helpers/network/) covers the parsing it needs.
