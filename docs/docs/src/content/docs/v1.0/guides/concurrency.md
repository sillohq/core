...
async def handler(ctx: HttpContext):
    thumb = resize(await ctx.body)      # blocks the loop for 200 ms
```
...
answer. That is what a [queue worker](/v1.0/guides/work/queue/) gives you, and
...
```bash
PYTHONASYNCIODEBUG=1 sillo dev
```
...
callback that logs, or use [`BackgroundTask`](/v1.0/guides/work/background/), which
...
- [Background Tasks](/v1.0/guides/work/background/): managed fire-and-forget with
  result tracking
- [Work Overview](/v1.0/guides/work/): moving heavy work out of the web process
  entirely
- [HTTP Client](/v1.0/guides/http/client/): timeouts, pooling, and gathering
  upstream calls
- [Async helpers](/v1.0/guides/helpers/async/): the utilities that complement these
  primitives
- [Middleware](/v1.0/guides/middleware/): where a blocking call hurts most
