...
async def home(ctx: HttpContext):
    req_id = getattr(ctx.state, "request_id", None)
    return {"message": "Hello with HttpContext ID!", "request_id": req_id}
```
...
req_id = get_or_generate_request_id(ctx)
...
def some_deep_helper():
    # RequestContext.current() returns the active context for this ctx,
...
async def logging_middleware(ctx: HttpContext, call_next):
    request_id = getattr(ctx.state, "request_id", "unknown")
    logger = logging.LoggerAdapter(logging.getLogger(__name__), {"request_id": request_id})
    logger.info(f"Processing ctx: {ctx.method} {ctx.url.path}")
...
async def tracing_middleware(ctx: HttpContext, call_next):
    request_id = getattr(ctx.state, "request_id", None)
...
async def call_external_api(ctx: HttpContext):
    request_id = get_request_id_from_request(ctx)
...
async def structured_logging(ctx: HttpContext, call_next):
    request_id = getattr(ctx.state, "request_id", "unknown")
    logger = logging.LoggerAdapter(logging.getLogger(__name__), {"request_id": request_id})
    start_time = time.time()
    logger.info(f"Request started: {ctx.method} {ctx.url.path}")
    try:
        response = await call_next()
...
async def get_user(ctx: HttpContext, user_id: int):
    ...
    async with asyncpg.connect("postgresql://...") as conn:
        user = await conn.fetchrow(
            "SELECT * FROM users WHERE id = $1 -- HttpContext ID: $2",
            user_id, request_id,
        )
...
async def start_processing(ctx: HttpContext):
    request_id = get_request_id_from_request(ctx)
    data = await ctx.json
...
async def http_exception_handler(ctx: HttpContext, exc):
    request_id = get_request_id_from_request(ctx)
    return {
        "error": exc.detail,
...
2. sillo constructs a `HttpContext`
...
|---|---|
| Run on every request, regardless of route | [Middleware](/v1.0/guides/middleware/) |
| Produce a value the handler uses | [A dependency](/v1.0/guides/dependency-injection/) |
| Reject bad input | [Validation](/v1.0/guides/validation/) |
| Run once per process | [A lifespan hook](/v1.0/guides/startups-and-shutdowns/) |
| Do work after responding | [A background task](/v1.0/guides/work/background/) |
...
response](/v1.0/guides/streaming-response/) returns from the handler before the body
...
A [WebSocket](/v1.0/guides/websockets/) leaves the request lifecycle entirely
...
- [Middleware](/v1.0/guides/middleware/): the layers wrapping every request
- [Routing](/v1.0/guides/routing/): how a path becomes a handler
- [Validation](/v1.0/guides/validation/): where input is checked
- [Handlers](/v1.0/guides/handlers/): what runs at the centre
- [Concurrency](/v1.0/guides/concurrency/): what the loop is doing meanwhile
- [Startup & Shutdown](/v1.0/guides/startups-and-shutdowns/): what happens before
  the first request
