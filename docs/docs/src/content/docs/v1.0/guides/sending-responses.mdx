...
description: "Return the value and sillo encodes it. What each type becomes, and the builders in sillo.responses for when you need a status, a header, a redirect, a file or a stream."
head:
- tag: meta
  attrs:
    property: og:title
...
- tag: meta
  attrs:
    ...
    content: "Return the value and sillo encodes it. What each type becomes, and the builders in sillo.responses for when you need a status, a header, a redirect, a file or a stream."
---
...
**Return the value. That is the whole default.**
...
That is a complete handler. sillo encodes the return value and sends it with a
...
The rule has two branches, and it is worth knowing which one you are on,
because the second surprises people.
...
<Aside type="caution" title="A returned string is text/plain, not HTML">

`return "<h1>Hi</h1>"` sends `text/plain`, so the browser shows the tags rather
than a heading. The encoder has no way to tell markup from prose, and guessing
wrong in the other direction — serving user text as HTML — is an XSS bug, so it
does not guess.

When you mean HTML, say so with `html("<h1>Hi</h1>")`.
...
```py
return datetime(2026, 1, 1)             # text/plain: 2026-01-01T00:00:00
return {"at": datetime(2026, 1, 1)}     # application/json: {"at":"2026-01-01T00:00:00"}
```

##  When the return value is not enough

A returned value carries a body and nothing else. These need a response object:
...
A `response_model` applies to a returned value exactly as it does to a built
...
async def me(ctx: HttpContext):
    return {"id": 1, "name": "Ada", "password_hash": "..."}
    # sends {"id":1,"name":"Ada"}
```

A handler that returns a built response is *not* filtered — building the
response says the status, headers and body are your business. See
...
| | |
|---|---|
| `json(data)` | `application/json`, using the framework encoder — what a bare `return data` gives you, plus somewhere to hang options |
