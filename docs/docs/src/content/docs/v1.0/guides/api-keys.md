...
async def data(ctx: HttpContext):
    return {"called_by": ctx.user.identity}
```
...
async def data(ctx: HttpContext):
    ...
    if apikey is None or "read:data" not in (apikey.scopes or []):
        return json({"error": "insufficient scope"}, status_code=403)
    ...
```
...
- [Users & User Models](/v1.0/guides/users/): `ApiKeyUserMixin` wiring
- [JWT](/v1.0/guides/jwt-auth/) · [Sessions](/v1.0/guides/session-auth/)
