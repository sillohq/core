...
```python
from sillo import SilloApp, HttpContext, redirect
...
async def start(ctx: HttpContext):
    authorize = authorize_url(google, return_to=ctx.query_params.get("next"))
...
async def finish(ctx: HttpContext):
    try:
        ...
        return redirect(f"/login?error={exc.code}")
...
:::caution
**Set the cookie on the response you return.** `redirect(...).set_cookie(...)`
works, because `set_cookie` is a method on the response `redirect()` built.
Building one response, setting a cookie on it, and then returning a *different*
one sends the one without the cookie.
:::
...
[The security model](/v1.0/guides/oauth/security/) for why, and what that buys.
...
redirect(authorize.url).set_cookie(**authorize.cookie_kwargs(secure=False))
```

##  Where next

- [Providers](/v1.0/guides/oauth/providers/): the four shipped, and how to add any
  other.
- [Persisting the login](/v1.0/guides/oauth/persisting-logins/): session, JWT,
  account linking, or nothing.
- [OAuth in OpenAPI](/v1.0/guides/oauth/openapi/): what your API reference should
  say about all this.
- [The security model](/v1.0/guides/oauth/security/): state, PKCE, and what is
