...
Every attribute is stated rather than left to a default. `set_cookie` defaults
`secure` off, which is right for local development and wrong for this cookie —
and a default that is right in one environment and wrong in the other is
exactly the kind that goes unnoticed until it is a finding.
...
- **Nothing here authenticates later requests.** Once you have persisted the
  ...
  login](/v1.0/guides/oauth/persisting-logins/).
