...
async def update_user(ctx: HttpContext, user_id: str):
    user = await User.get_or_none(id=user_id)
    if user is None:
        return json({"error": "Not found"}, status_code=404)
    await user.update_from_dict(
        ctx.validated_data.model_dump(exclude_unset=True)
    )
...
scope. See [Scopes & Events](/v1.0/guides/record/scopes-events/).
...
a [transaction](/v1.0/guides/record/transactions-factories/).
...
| `SlugField` | `CharField` with a `source_field` argument | Fills a blank slug from that field on save |
| `ULIDField` | `CharField(max_length=26, pk=True)` | **Never generates a value** |
| `PasswordField` | `CharField` that hashes on write | Works; not exported from `sillo.record` |

`PasswordField` hashes plaintext in `to_db_value` and passes through a value
that is already a hash, whichever scheme produced it, so re-saving a loaded row
...
`SlugField(source_field="title")` fills the slug from `title` when a row is
saved without one. An explicitly set slug is always kept.
...
- [Record Overview](/v1.0/guides/record/): setup, configuration, and connection
  lifecycle
- [Scopes & Events](/v1.0/guides/record/scopes-events/): global scopes for automatic
  soft-delete filtering
- [Casting & Collections](/v1.0/guides/record/casting-collections/): `_casts` and
  result manipulation
- [Exception Handlers & Pydantic](/v1.0/guides/record/exceptions-pydantic/):
  generating schemas from these models
- [Hashing helpers](/v1.0/guides/helpers/hashing/): what `PasswordField` uses
  internally
- [Serialization](/v1.0/guides/serialization/): how sillo encodes handler return
  values
