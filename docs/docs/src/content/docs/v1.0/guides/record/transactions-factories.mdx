...
async def place_order(ctx: HttpContext):
    ...
    return json(order.to_dict(), status_code=201)
```
...
- [Models & Mixins](/v1.0/guides/record/models/): `bulk_create`, `upsert`, and
  validation hooks
- [Scopes & Events](/v1.0/guides/record/scopes-events/): why lifecycle events do not
  fire inside `create()`
- [Migrations & Seeding](/v1.0/guides/record/migrations/): seeders and fixtures for
  non-test data
- [Exception Handlers & Pydantic](/v1.0/guides/record/exceptions-pydantic/): turning
  `IntegrityError` into a 409
- [Concurrency](/v1.0/guides/concurrency/): how sillo schedules the tasks these run
  in
