...
application (session authentication against a real user model, migrations, a
JSON API, a queue) that you copy and make yours. Its CI boots it and calls every route on every push, on three Python
versions.
...
Come back here when you want the wiring (sessions, the user model, the console)
...
| **Pages** | One HTML page and a stylesheet, with `/static` served in development |
...
- **No second user model.** One `User`, and everything authenticates against
  it. See [Users & Authentication](/v1.0/guides/start/authentication/).
...
2. **Installing dependencies is not the same as creating the database.**
   Installing gets you the `sillo` command; `sillo db:migrate` is what
   creates the schema.

3. **The database file is gitignored, the migrations are not.** Commit
