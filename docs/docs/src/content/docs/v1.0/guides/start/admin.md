...
@admin.register(Post)
class PostAdmin(ModelAdmin):
    def has_add_permission(self, ctx: HttpContext) -> bool:
        return ctx.user.is_superuser

    def has_change_permission(self, ctx: HttpContext) -> bool:
        return True

    def has_delete_permission(self, ctx: HttpContext) -> bool:
        return ctx.user.is_superuser
```
...
class HeaderAuth(AuthBackend):
    async def authenticate(self, ctx: HttpContext) -> bool:
        return ctx.headers.get("X-Forwarded-Email") in ALLOWED

    async def get_user(self, ctx: HttpContext):
        return {"id": ctx.headers.get("X-Forwarded-Email"), "display_name": "SSO"}
...
5. **Registering the admin after the middleware block** makes every admin
   ...
   [Project Structure](/v1.0/guides/start/structure/#appbootstrappy).

##  Related

- [Users & Authentication](/v1.0/guides/start/authentication/): the model the admin
  signs in
- [Database & Migrations](/v1.0/guides/start/database/): what `MODEL_MODULES`
  decides
- [Project Structure](/v1.0/guides/start/structure/): where `app/admin.py` sits
- [Middleware](/v1.0/guides/middleware/): why registration order matters
