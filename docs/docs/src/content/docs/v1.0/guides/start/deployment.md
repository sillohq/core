...
@router.get("/ready")
async def ready(ctx: HttpContext):
    manager = ctx.app.state["record"]
    return json({"ready": await manager.health()}, status_code=200)
```
...
- [Creating a Project](/v1.0/guides/start/): what you are deploying
- [Database & Migrations](/v1.0/guides/start/database/): migrating safely
- [Background Work](/v1.0/guides/start/background-work/): workers in production
- [Testing](/v1.0/guides/start/testing/): the suite and the smoke check
- [The Console](/v1.0/guides/start/console/): the commands a deployment runs
