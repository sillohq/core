...
async def delete_user(ctx: HttpContext, id: int): ...
```
...
See [DB-backed permissions](/v1.0/guides/users/#db-backed-permissions) for the full API.
...
async def feed(ctx: HttpContext):
    if ctx.user.is_authenticated:
        return {"feed": "personalized", "user": ctx.user.display_name}
    return {"feed": "public"}
```
...
async def health(ctx: HttpContext):
    return {"service": ctx.user.display_name}
```
...
class OrgScoped(useAuth):
    ...
    async def authenticate(self, ctx: HttpContext) -> bool:
        if not await super().authenticate(ctx):
            return False
        org_id = ctx.path_params[self.org_id_param]
...
async def org_members(ctx: HttpContext, org_id: str): ...
```
...
- [Users & User Models](/v1.0/guides/users/): `has_permission`, `UserProtocol`
...
from sillo import HttpContext, json, Path, Router
...
async def get_order(ctx: HttpContext, order_id: int = Path(type=int)):
    order = await Order.get_or_none(id=order_id, owner_id=ctx.user.id)
    if order is None:
        raise HTTPException(status_code=404)
...
async def cancel_order(ctx: HttpContext, order_id: int = Path(type=int)):
    deleted = await Order.filter(id=order_id, owner_id=ctx.user.id).delete()
    if not deleted:
        raise HTTPException(status_code=404)
