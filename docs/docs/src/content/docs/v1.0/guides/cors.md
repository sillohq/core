...
cors_config = CorsConfig(
        ...
        custom_error_messages={
            ...
            "missing_origin": "The ctx is missing an origin."
        }
    )
app.use(CORSMiddleware(config=cors_config))
```
...
- [CSRF](/v1.0/guides/csrf/): protect cookie-auth state-changing requests from
  cross-site forgery
...
This is why a form POST is a [CSRF](/v1.0/guides/csrf/) concern while a JSON
...
- [Middleware](/v1.0/guides/middleware/): where CORS belongs in the stack
- [CSRF](/v1.0/guides/csrf/): the other cross-origin concern, and a different one
- [Headers](/v1.0/guides/headers/): the response headers involved
