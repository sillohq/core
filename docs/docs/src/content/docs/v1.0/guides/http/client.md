...
from httpx import HttpContext

class TimingMiddleware(HTTPMiddleware):
    async def handle(self, ctx: HttpContext, next_call):
        import time
        start = time.monotonic()
        async for response in next_call(ctx):
...
```python
from sillo.http.client import (
    extract_response_summary,   # human-readable summary of an httpx BaseResponse
...
[retry helpers guide](/v1.0/guides/helpers/retry/).
...
- [Retry helpers](/v1.0/guides/helpers/retry/): backoff, jitter, and retry policy in
  detail
- [Cache](/v1.0/guides/cache/): the backends the response cache uses
- [Network helpers](/v1.0/guides/helpers/network/): client IP handling and SSRF
  considerations
- [Concurrency](/v1.0/guides/concurrency/): gathering independent calls
- [Startup & Shutdown](/v1.0/guides/startups-and-shutdowns/): where to open and
  close the client
- [Error Handling](/v1.0/guides/error-handling/): turning upstream failures into
