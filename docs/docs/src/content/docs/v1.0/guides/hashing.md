...
@app.post("/login")
async def login(ctx: HttpContext, username: str, password: str):
...
# Use in registration
@app.post("/register")
async def register(ctx: HttpContext, password: str = Query(...)):
    errors = validate_password(password)
    if errors:
        return json({"errors": errors}, status_code=400)
    
    user = User(username="john")
    user.set_password(password)
    await user.save()
    return json({"user_id": user.id}, status_code=201)
```
...
```python
from sillo import SilloApp, HttpContext, json, Query
from sillo.hashing import validate_password
from sillo.users import User
...
async def request_reset(ctx: HttpContext, email: str = Query(...)):
...
- [User Authentication](/v1.0/guides/authentication) - Complete auth guide
- [User Model](/v1.0/guides/users) - sillo.users documentation
- [Permissions](/v1.0/guides/permissions) - Access control
- [Sessions](/v1.0/guides/sessions) - Session management
- [Security](/v1.0/guides/security) - Security best practices
