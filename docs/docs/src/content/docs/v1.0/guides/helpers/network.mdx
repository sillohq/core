...
<Aside type="danger" title="The default trusts every header from every peer">
...
**Always pass `trusted_proxies` in production.** The value is the address of
*your own* infrastructure, the load balancer or proxy that sits directly in
front of the app:
...
Scanning right-to-left skips the hops you control and stops at the first
address that could not have been forged past them. Taking the leftmost value
instead (which many hand-rolled implementations do) means trusting whatever the
client typed.
...
<Aside type="caution" title="Header lookup is lowercase">
...
<Aside type="caution" title="The result is not validated">
...
async def rate_limit(ctx: HttpContext, call_next):
    ...
    if await too_many_requests(client):
        return json({"error": "Rate limited"}, status_code=429)
    return await call_next()
```

For production rate limiting use the built-in middleware in [Rate Limiting](/v1.0/guides/rate-limiting/) rather than writing your own; the example above is to show where the IP comes from.
...
<Aside type="caution" title="Resolve-then-fetch is still racy">

The example above resolves the hostname, checks the address, then fetches by
URL, and the fetch resolves again. An attacker controlling the DNS record can
return a public address for the first lookup and an internal one for the
second. This is DNS rebinding.
...
<Aside type="danger" title="A CDN in trusted_proxies is only safe if the origin is locked down">
...
Restrict the origin at the network layer so only the CDN can reach it: a
security group, a firewall rule, or an authenticated origin pull. The header
trust is only as strong as that restriction.
...
```python
client = get_client_ip(ctx.headers, ctx.client.host, trusted_proxies=TRUSTED_PROXIES)
logger.info("peer=%s resolved=%s xff=%s",
            ctx.client.host, client, ctx.headers.get("x-forwarded-for"))
```
...
Integer form is useful for storing addresses in a plain integer column and for
range queries. `WHERE ip_int BETWEEN ? AND ?` uses an index, whereas string
comparison on dotted quads does not sort correctly (`"9.0.0.0"` sorts after
`"10.0.0.0"`).
...
Every function parses its input with the stdlib `ipaddress` module, which
allocates an object per call. That is microseconds, irrelevant once per
request, noticeable in a loop over a million rows.
...
- [Rate Limiting](/v1.0/guides/rate-limiting/): the main consumer of client IP
- [Request Information](/v1.0/guides/request-info/): reading headers and the peer
  address
- [Security](/v1.0/guides/security/): trusted hosts and the wider hardening story
- [Middleware](/v1.0/guides/middleware/): where IP resolution usually belongs
