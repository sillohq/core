...
# resolve ctx.user from a bearer token
...
See [Documenting Authentication](/v1.0/guides/openapi/authentication-documentation/)
...
# GET /me  with  Authorization: Bearer <token>  →  ctx.user is alice
```
...
On failure it raises `AuthenticationFailed` (401) or `PermissionDenied` (403). See [Protecting Routes](/v1.0/guides/protecting-routes/) for the full gate reference.
...
- [JWT Authentication](/v1.0/guides/jwt-auth/)
- [Session Authentication](/v1.0/guides/session-auth/)
- [API Keys](/v1.0/guides/api-keys/)
...
See [Users & User Models](/v1.0/guides/users/) for the built-in `User`, building custom users, permissions, and password hashing.
...
class HeaderBackend(AuthenticationBackend):
    async def authenticate(self, ctx: HttpContext):
        token = ctx.headers.get("X-Service-Token")
        if not token:
...
async def on_401(ctx: HttpContext, exc):
...
async def orders(ctx: HttpContext): ...
```

Each hook is called as `hook(ctx)` — sync or async, your choice —
...
- [Protecting Routes](/v1.0/guides/protecting-routes/): every `useAuth` option,
  scopes, permissions, subclassing
- [Users & User Models](/v1.0/guides/users/): `User`, `UserProtocol`,
  `UserBaseModel`, `SimpleUser`, passwords
- [JWT Authentication](/v1.0/guides/jwt-auth/): issuing and verifying tokens
- [Session Authentication](/v1.0/guides/session-auth/): `SessionGuard` and cookie
  login
- [API Keys](/v1.0/guides/api-keys/): scoped, hashed keys
...
[CSRF](/v1.0/guides/csrf/) exposure, which is a solved problem.
...
**Machine-to-machine.** [API keys](/v1.0/guides/api-keys/). Long-lived,
...
helpers](/v1.0/guides/helpers/hashing/).
...
[Rate Limiting](/v1.0/guides/rate-limiting/).
...
- [Protecting Routes](/v1.0/guides/protecting-routes/): declaring the requirement
- [Permissions](/v1.0/guides/permissions/): what an authenticated caller may do
- [Sessions](/v1.0/guides/sessions/): cookie-based credentials
- [JWT Auth](/v1.0/guides/jwt-auth/): token-based credentials
- [API Keys](/v1.0/guides/api-keys/): machine-to-machine credentials
- [Hashing helpers](/v1.0/guides/helpers/hashing/): storing passwords correctly
- [Rate Limiting](/v1.0/guides/rate-limiting/): protecting the login path
