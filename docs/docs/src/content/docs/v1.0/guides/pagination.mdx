...
```python {6}
from sillo import SilloApp, HttpContext, paginate
...
async def get_items(ctx: HttpContext):
    ...
    return paginate(ctx, sample_data)
```

For async endpoints, use `response.apaginate()`:

```python {6}
from sillo import SilloApp, HttpContext, apaginate
...
async def get_items(ctx: HttpContext):
    sample_data = [{"id": i, "content": f"Item {i}"} for i in range(1, 101)]
    return apaginate(ctx, sample_data)
```
...
<Aside type="tip" title="Navigation Links">
...
async def get_items(ctx: HttpContext):
    sample_data = [{"id": i, "content": f"Item {i}"} for i in range(1, 101)]
    return paginate(ctx, sample_data, strategy="page_number")
```
...
async def get_items(ctx: HttpContext):
    sample_data = [{"id": i, "content": f"Item {i}"} for i in range(1, 101)]
    return paginate(ctx, sample_data, strategy="limit_offset")
```
...
async def get_items(ctx: HttpContext):
    sample_data = [{"id": i, "content": f"Item {i}"} for i in range(1, 101)]
    return paginate(ctx, sample_data, strategy="cursor")
```
...
<Aside type="tip" title="Cursor Encoding">
...
async def get_items_page(ctx: HttpContext):
    ...
    return paginate(ctx, data, strategy=strategy)
...
async def get_products(ctx: HttpContext):
    ...
    min_price = ctx.query_params.get("min_price")
    ...
    # Sort results
    sort_by = ctx.query_params.get("sort", "id")
    reverse = ctx.query_params.get("order") == "desc"
    filtered_products.sort(key=lambda x: x.get(sort_by, 0), reverse=reverse)
    
    return paginate(ctx, filtered_products)
```
...
<Aside type="tip" title="Preserving Query Parameters">
...
<Aside title="Automatic Selection">
...
async def get_items(ctx: HttpContext):
    ...
    query = Item.all()
    return paginate(ctx, query, data_handler=TortoiseDataHandler(query))
```
...
@app.get("/items-seek")
async def get_items_seek(ctx: HttpContext):
    data = [{"id": i, "name": f"Item {i}"} for i in range(1, 101)]
    return paginate(ctx, data, strategy=SeekPagination())
```
...
`OFFSET 100000` makes the database read and discard a hundred thousand rows
before returning twenty. Page one is instant, page five thousand is a table
scan, and no index fixes it. The offset must be counted.
...
The tradeoff is that keyset paging cannot jump to an arbitrary page. There is
no "page 47". For feeds, timelines, and infinite scroll that is no loss. For an
admin table where users click page numbers, offset is the right tool and the
page count is bounded by the filter.
...
Three ways out, in increasing order of how much you give up. Cache the count
for a few seconds. Most users cannot tell a stale total from a fresh one.
Return an approximate count from table statistics for unfiltered lists. Or drop
the total entirely and return only "is there a next page", which is all an
infinite-scroll UI needs and costs one extra row rather than a full count.
