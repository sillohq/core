...
async def get_premium_content(ctx: HttpContext):
    if not ctx.user.has_premium:
        raise PaymentRequiredException("Upgrade to premium to access this content")
    return {"message": "Premium content available"}
...
async def handle_payment_required_exception(ctx: HttpContext, exception):
    return json({"error": "Payment required"}, status_code=402)
...
async def server_error_handler(ctx: HttpContext, exception):
...
The `if settings:` branch is unreachable, so every option below (`return_json`,
`custom_message`, `show_traceback`, `use_html`) has no effect. The handler
always uses its fallback: JSON output, and `debug = True`, which means the raw
`exception.detail` is returned rather than your custom message.
...
field. An internal error should name nothing: a stack trace, a SQL fragment, or
a file path in a production response is reconnaissance you handed over for
free.
...
async def unhandled(ctx: HttpContext, exc):
    request_id = ctx.headers.get("x-request-id", "-")
    logger.exception("unhandled error [%s] on %s", request_id, ctx.url.path)
    return json(
...
The `request_id` is the important half. It gives a user something to quote to
support, and it gives you a key to find the full traceback in your logs, all
the debuggability, none of the disclosure.
