...
async def upload_avatar(ctx: HttpContext):
    ...
    return json({"key": stored.key, "size": stored.size})
```
...
[Instance Registry](/v1.0/advanced/context-binding/) — nothing here depends on
knowing.)
...
- [Storage internals](/v1.0/advanced/storage/) — the driver contract, the design
  decisions, and where "S3-compatible" leaks
- [File uploads](/v1.0/guides/file-upload/) — parsing multipart before it reaches a
  bucket
