...
async def home(ctx: HttpContext):
    accepts_info = getattr(ctx.state, "accepts", {})
    return {
...
@app.get("/api/content")
async def get_content(ctx: HttpContext):
...
@app.get("/greetings")
async def get_greetings(ctx: HttpContext):
...
async def debug_accepts(ctx: HttpContext):
    accepts = getattr(ctx, "accepts", {})
    return {
...
@app.get("/negotiate")
async def negotiate_example(ctx: HttpContext):
...
class CustomNegotiationMiddleware(ContentNegotiationMiddleware):
    ...
    def negotiate_content_type(self, ctx: HttpContext, available_types, default_type=None):
        accept_header = ctx.headers.get("Accept", "")
            ...
            if custom_type in accept_header:
                for fallback in fallback_types:
                    if fallback in available_types:
                        return fallback
        return super().negotiate_content_type(ctx, available_types, default_type)
...
@app.get("/messages")
async def get_messages(ctx: HttpContext):
    available_messages = {
...
def test_negotiates_json():
    ...
    @app.get("/data")
    async def data(ctx: HttpContext):
        return {"ok": True}
...
def test_stores_accepts_info():
    ...
    @app.get("/debug")
    async def debug(ctx: HttpContext):
        return {"types": [i.value for i in ctx.state.accepts["accept"]]}
...
async def debug_headers(ctx: HttpContext):
    return {
        "accept": ctx.headers.get("Accept"),
        "accept_language": ctx.headers.get("Accept-Language"),
        "accept_charset": ctx.headers.get("Accept-Charset"),
