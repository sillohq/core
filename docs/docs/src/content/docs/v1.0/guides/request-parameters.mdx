...
`Depend(get_request=True)` injects the raw `HttpContext` into a dependency,
leaving the extraction to you:

```python
from sillo import Depend

def get_user_data(ctx: HttpContext = Depend(get_request=True)):
...
handlers get the context that way. Without `Depend(get_request=True)` it is
