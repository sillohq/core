...
def get_preferences(_, theme: str = Cookie("dark"), lang: str = Cookie("en")):
...
@app.get("/dashboard")
async def dashboard(ctx: HttpContext, gathered: dict = Depend(collect_all)):
    return gathered
```
...
A dependency's first positional parameter is the active context — an
`HttpContext` on an HTTP route, a `WebSocketContext` on a WebSocket route.
Take it and do the extraction yourself:

```python
from sillo import Depend

def get_user_data(ctx: HttpContext):
    page = ctx.query_params.get("page", "1")
    authorization = ctx.headers.get("Authorization")
    return {"page": page, "token": authorization}
```

Every dependency is called with the context first, just like a handler. One
that does not need it names the parameter `_`.
