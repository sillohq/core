...
<Aside type="caution" title="Set headers on the response you return">

`set_header` and the rest are methods on a response, so there is nothing to
call them on until `stream(...)` has built one.
...
async def stream_data(ctx: HttpContext):
    # ❌ WRONG: there is no `response` here — the name is undefined
    response.set_header("X-Streaming", "enabled")
    async def generate():
        for i in range(5):
...
```python
from sillo import SilloApp, HttpContext, stream
...
async def stream_data(ctx: HttpContext):
    ...
    return stream(generate(), content_type="text/plain")
```
...
@app.get("/chunked")
async def chunked_response(ctx: HttpContext):
    async def generate():
        yield "First chunk\n"
        await asyncio.sleep(1)
        yield "Second chunk\n"
...
`response.sse()` takes an async iterable and handles the encoding, the
headers, and the keepalive:

```python
from sillo import HttpContext, sse, BaseResponse
...
Yield whatever is convenient. A `dict` or list is JSON-encoded, a `str` is sent
unchanged, and a `ServerSentEvent` gives you the rest of the protocol:

```python
from sillo.http.sse import ServerSentEvent
...
events.addEventListener("tick", (e) => console.log(JSON.parse(e.data)))
```

####  Keepalive

An idle SSE connection gets closed by whatever sits between you and the
browser: nginx's `proxy_read_timeout` defaults to 60 seconds, and most load
balancers are similar. `response.sse()` sends a comment line every 15 seconds
of silence to prevent that. Clients ignore comments, so it costs the page
nothing.

```python
from sillo import sse

return sse(source(), keepalive=30.0)   # or None to disable
```

####  Reconnection

`EventSource` reconnects on its own, and sends the last `id` it saw as the
`Last-Event-ID` header. Read it to resume rather than replay:

```python
from sillo.http.sse import last_event_id
...
async def events(ctx: HttpContext):
    ...
    async def source():
        async for row in changes_since(since):
            yield ServerSentEvent(data=row, id=str(row["id"]))

    # How long the browser waits before reconnecting, in milliseconds.
    return sse(source(), retry=5000)
```

:::caution[TestClient cannot verify SSE timing]
The ASGI test transport buffers the whole response, so every event arrives at
once and a test cannot tell incremental delivery from a batch. Assert on the
payload with `TestClient`; verify streaming behaviour against a real server.
:::

####  Headers

Three are set for you beyond `text/event-stream`: `no-cache` keeps the stream
out of caches, `no-transform` stops a proxy re-encoding it, and
`x-accel-buffering: no` disables nginx's response buffering, without which
events are held back and delivered in batches. Anything you pass in `headers`
overrides them.
...
from sillo import HttpContext, json, stream

@app.get("/download/{filename}")
async def download_file(ctx: HttpContext, filename: str):
...
@app.get("/safe-stream")
async def safe_stream(ctx: HttpContext):
    async def generate():
        try:
            for i in range(10):
                if i == 5:
...
async def data_pipeline(ctx: HttpContext):
    """
...
**A streamed response** is one HTTP response whose body arrives in pieces.
Right for large payloads (a CSV export, a log tail, a generated file) where you
want constant memory rather than building the whole body first. The connection
...
**A WebSocket** is a persistent bidirectional connection. Right only when the
client also needs to push at arbitrary times: chat, collaborative editing,
multiplayer. See [WebSockets](/v1.0/guides/websockets/).
...
**Compression** buffers by nature. A gzip encoder needs input before it emits
output. Disable compression on streaming endpoints or accept the latency.

**Idle timeouts** close a connection that sends nothing for too long, typically
between 30 and 120 seconds. A stream with quiet periods needs a keepalive, for
SSE, a comment line every 20 to 30 seconds is the conventional answer.
...
If the client stops reading and you keep producing, the data queues in buffers
until something runs out of memory. An `async` generator that awaits its writes
gets backpressure for free. The await does not return until the transport
accepts the chunk. A generator that produces without awaiting does not.
