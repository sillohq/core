...
async def create_user(ctx: HttpContext):
    ...
    try:
        ...
        return json({"error": "Invalid data", "details": str(e)}, status=400)
    
    # Create user logic here
    return json({"id": 123, "username": username}, status=201)
```
...
async def create_order(ctx: HttpContext):
    ...
    try:
        ...
        return json({"order_id": 12345}, status=201)
    except Exception as e:
        return json({"error": "Order validation failed", "details": str(e)}, status=400)
```
...
async def update_user(ctx: HttpContext, user_id: int):
    ...
    try:
        ...
        return json({"error": "Update validation failed"}, status=400)
```
