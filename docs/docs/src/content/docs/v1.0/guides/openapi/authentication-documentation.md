...
async def flexible_auth_endpoint(ctx: HttpContext):
...
async def get_content(ctx: HttpContext, content_id: int):
...
async def get_secure_data(ctx: HttpContext):
    auth_header = ctx.headers.get('Authorization')
...
async def auth_middleware(ctx: HttpContext, next_call):
    """Authentication middleware for protected endpoints"""
    
    # Skip auth for public endpoints
    if ctx.url.path in ['/health', '/docs', '/openapi.json']:
