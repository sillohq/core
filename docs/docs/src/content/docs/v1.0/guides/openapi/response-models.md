...
async def get_user_with_errors(ctx: HttpContext, user_id: int):
    try:
...
async def list_users(ctx: HttpContext):
    ...
    per_page = int(ctx.query_params.get('per_page', 20))
...
async def get_active_users(ctx: HttpContext):
    users = [
...
async def get_detailed_user(ctx: HttpContext, user_id: int):
...
async def get_user_wrapped(ctx: HttpContext, user_id: int):
    try:
        user = UserResponse(
            id=user_id,
            username="johndoe",
...
async def get_account(ctx: HttpContext, account_id: int):
...
async def export_user_data(ctx: HttpContext, user_id: int):
...
async def get_order(ctx: HttpContext): ...
```
