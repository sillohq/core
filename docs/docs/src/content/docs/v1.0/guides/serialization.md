...
@app.get("/price")
async def price(ctx: HttpContext):
...
async def order(ctx: HttpContext, order_id: int):
...
def test_per_call_overrides_global():
    ...
    @app.get("/override")
    async def override(ctx: HttpContext):
        return json(
            Vector(7, 8),
...
- [Sending Responses](/v1.0/guides/sending-responses/): `response.json(...)` and
  other response builders
- [Dependency Injection](/v1.0/guides/dependency-injection/): injecting custom-typed
  values
- [Request Inputs](/v1.0/guides/request-inputs/): parsing incoming data before you
...
[`response_model`](/v1.0/guides/validation/response-models/), which drops
...
models. See [Request Bodies](/v1.0/guides/validation/request-bodies/) for the
