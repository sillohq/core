...
When a client makes a request to your sillo application, any cookies sent by the browser are automatically parsed and made available through the `ctx.cookies` property.
...
@app.get("/")
async def read_cookie(ctx: HttpContext):
    auth_token = ctx.cookies.get("auth_token")
    return {"token": auth_token}
```

<Aside type="tip">

The `ctx.cookies` property returns a dictionary where keys are cookie names and values are the corresponding cookie values.
...
async def set_cookie(ctx: HttpContext):
    return json({"status": "Cookie set"}).set_cookie(
        key="user_id",
        value="12345",
...
async def logout(ctx: HttpContext):
    return json({"status": "Logged out"}).delete_cookie("auth_token")
```
...
@app.get("/remember-me")
async def remember_me(ctx: HttpContext):
    return text("Cookie set for 10 years").set_permanent_cookie(
        key="remember_me",
        value="true"
    )
```
...
async def multi_cookie(ctx: HttpContext):
    cookies = [
        {"key": "user", "value": "john", "max_age": 3600},
        {"key": "theme", "value": "dark", "max_age": 86400}
    ]
    return json({"status": "Cookies set"}).set_cookies(cookies)
```
...
async def login(ctx: HttpContext):
    ...
    if validate_credentials(data["username"], data["password"]):
        return json({"status": "Login successful"}).set_cookie(
            key="auth_token",
...
async def protected_route(ctx: HttpContext):
    if not ctx.cookies.get("auth_token"):
        return json({"error": "Unauthorized"}).status(401)
    
    # Verify token (pseudo-code)
    user = verify_token(ctx.cookies["auth_token"])
    if not user:
        return json({"error": "Invalid token"}).status(401).delete_cookie("auth_token")
    
    return json({"message": f"Welcome {user}"})
...
`Max-Age` or `Expires` decides persistence. Without either, the cookie is a
session cookie and dies with the browser process: which, given modern browsers
restore sessions on restart, is less reliable than it sounds.
...
Every cookie for a domain is sent on **every request to that domain**. Images,
stylesheets, API calls, all of them. Ten kilobytes of cookies is ten kilobytes
of upload per request, which on a mobile connection is measurable latency on
every asset.

Two consequences worth designing around. Serve static assets from a cookieless
domain or a CDN, so asset requests carry nothing. And keep an identifier in the
cookie rather than data, with the data server-side, which is the same argument
as for session storage, arriving from a different direction.
...
Deleting requires matching attributes. A cookie set with `path="/app"` is not
removed by a delete on `path="/"`. The browser treats them as different
cookies, and the stale one keeps being sent.
...
- [Sessions](/v1.0/guides/sessions/): what most cookies carry
- [Session Auth](/v1.0/guides/session-auth/): the authentication flow
- [CSRF](/v1.0/guides/csrf/): why cookies need `SameSite` and a token
- [Headers](/v1.0/guides/headers/): `Set-Cookie` and header size limits
...
raw = ctx.cookies.get("prefs")
value = unsign_value(raw, secret) if raw else None
if value is None:
    ...   # tampered or absent — treat as no preference
```

Two things to be clear about. Signing proves the value came from you; it does
not hide it, so the contents remain readable by the user and by anything with
access to their disk. And a signed cookie has no built-in expiry. See the note
on `max_age` in [Crypto helpers](/v1.0/guides/helpers/crypto/), so if the value
should stop being valid, put a timestamp inside it and check that yourself.
...
architecture requires cross-site cookies, moving the API to a subdomain of the
site that uses it converts the problem into a same-site one.
...
For deletion, assert that a subsequent request with the old cookie is rejected,
not merely that a `Set-Cookie` clearing it was sent, which is true even when
the server-side session survives.


##  Summary

The attributes are the security model, and the defaults are the permissive
ones. Set `HttpOnly` and `Secure` on anything that is a credential,
`SameSite=Lax` on everything, and scope `Domain` and `Path` as narrowly as the
application allows. Keep an identifier in the cookie and the data on the
server: it avoids the size limit, the per-request upload cost, and the
disclosure that comes from a value the user can read.


##  Cookies and caching

A response carrying `Set-Cookie` must never be cached publicly. A shared cache
that stores it will hand the same cookie (and therefore the same session) to
the next person who requests that URL.
