...
A `Channel` is a thin wrapper around a `WebSocketContext` that adds three
...
channel.websocket     # the underlying WebSocketContext
```
...
[`WebSocketConsumer`](/v1.0/guides/websockets/consumer/) builds one per
...
from sillo.websockets import Channel, ChannelBox, WebSocketContext, WebSocketDisconnect
...
`WebSocketContext` instance, `expires` must be an `int` if given, and `payload_type`
...
`payload_type` decides which `WebSocketContext` method `_send` calls:
...
class UserChannel(Channel):
    def __init__(self, websocket: WebSocketContext, user_id: int, **kwargs):
...
class PresenceChannel(Channel):
    def __init__(self, websocket: WebSocketContext, user_id: int, display_name: str):
...
async def presence(websocket: WebSocketContext):
...
The division is: receive on the `WebSocketContext`, send through the `Channel` when
the message is a broadcast, and send on the `WebSocketContext` directly when the
...
- [WebSockets](/v1.0/guides/websockets/): the underlying connection
- [Consumers](/v1.0/guides/websockets/consumer/): where channels get built for you
- [Groups](/v1.0/guides/websockets/groups/): `ChannelBox`, broadcast, and history
- [Events](/v1.0/guides/websockets/events/): dispatching messages by type
