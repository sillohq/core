...
The core ships the connection itself, and nothing about talking to more than
one of them at a time:

| Layer | What it gives you | Where |
|---|---|---|
| `WebSocketContext` | The raw connection: accept, send, receive, close | This page |
| `Peer`, `Hub`, `RoomConsumer` | Rooms, presence, broadcast, replay | [`sillo-wire`](/packages/wire/) |

Rooms and fan-out moved out of the core in v1. The reason is a dependency
direction rather than size: the room layer needs a socket, a socket needs
nothing from the room layer, and fan-out is the part that grows a backend once
groups have to span more than one worker process.

```bash
pip install sillo-wire
```

Start here regardless. Most applications end up using a hub, but understanding
the raw connection first makes the rest obvious rather than magical.
...
infrastructure. See [Streaming Responses](/v1.0/guides/streaming-response/).
...
The handler takes the context first, exactly as an HTTP handler does — a
`WebSocketContext` rather than an `HttpContext`. It does not return anything;
...
Declared the same way as HTTP routes, and delivered the same way: as keyword
...
They are also on `websocket.path_params`, which is what you read when the
handler is generic over the route:

```python title="the same value, off the connection"
from sillo import WebSocketContext
...
A WebSocket handler's signature is analysed the same way an HTTP handler's is,
so it can declare `Depend(...)` parameters. The whole tree is resolved once,
before the handler runs — nothing is re-solved per message.

```python title="a dependency on a socket"
...
Generator dependencies work too: the part after `yield` runs when the
connection handler returns, so a resource opened for the socket is released
when the socket closes.

```python title="a resource scoped to the connection"
async def get_conn():
...
To reach the connection object itself from inside a dependency, use
`Depend(get_context=True)`. On a WebSocket route it injects the live
`WebSocketContext`; on an HTTP route the same marker injects the `HttpContext`.

```python title="the connection, inside a dependency"
from sillo import Depend, WebSocketContext


def client_ip(ctx: WebSocketContext = Depend(get_context=True)) -> str:
...
`WebSocketContext` tracks two states (`client_state` and `application_state`) and
...
| `path_params` / `query_params` / `headers` | mappings | Inherited from `BaseContext` |

##  Related

- [Wire](/packages/wire/): rooms, presence, broadcast and replay, in a package
  that installs alongside the core
- [Events](/v1.0/guides/websockets/events/): routing messages to handlers by type
- [Streaming Responses](/v1.0/guides/streaming-response/): SSE, for one-directional
  push
- [Routing](/v1.0/guides/routing/): how paths and parameters are matched
- [Dependency injection](/v1.0/guides/dependency-injection/): `Depend(...)` on a
  handler, HTTP or WebSocket
