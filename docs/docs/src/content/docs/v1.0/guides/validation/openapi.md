...
async def items(ctx: HttpContext):
    flag = ctx.query_params.get("legacy_flag")     # you extract it yourself
```
...
async def metrics(ctx: HttpContext):
    ...
```
...
| Path | What it serves |
| --- | --- |
| `/docs` | [Atlas](/v1.0/guides/openapi/documentation-ui/#atlas): sillo's own reference, with a request builder |
...
argument. See [Documentation UI](/v1.0/guides/openapi/documentation-ui/).
