...
```python
from sillo import SilloApp, HttpContext, Query, Header, Cookie, Path

app = SilloApp()

@app.get("/teams/{team_id}/items")
async def list_items(ctx: HttpContext,
...
`Form` and `File` are covered in [Forms and file uploads](/v1.0/guides/validation/forms-and-files/).
...
@app.get("/items")
async def search(ctx: HttpContext, tags=Query([], type=List[str])):
    return {"tags": tags}
```
...
async def get_item(ctx: HttpContext, item_id=Path(type=int, ge=1)):
    return {"id": item_id}
```
...
@app.get("/items")
async def list_items(ctx: HttpContext, pager=Depend(pagination)):
    return pager
```
...
@app.get("/a")
async def a(ctx: HttpContext, page=PAGE): ...

@app.get("/b")
async def b(ctx: HttpContext, offset=PAGE):   # reads ?offset=, not ?page=
    ...
```
...
@app.get("/products")
async def products(ctx: HttpContext, sort: str = Query("created", type=str)):
