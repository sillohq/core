...
```python
from sillo import SilloApp, HttpContext, json, RequestValidationError

app = SilloApp()

async def my_validation_handler(ctx: HttpContext, exc):
    return json(
...
async def redacted(ctx: HttpContext, exc):
    errors = []
    for err in exc.errors:
        err = dict(err)
...
```python
from sillo import HttpContext, json, ResponseValidationError

async def on_response_invalid(ctx: HttpContext, exc):
    alert_oncall(path=ctx.url.path, errors=exc.errors)
...
```python
from sillo import HttpContext, RequestValidationError
...
async def on_validation_error(ctx: HttpContext, exc):
    logger.info(
