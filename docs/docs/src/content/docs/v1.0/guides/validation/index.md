...
from sillo import SilloApp, HttpContext, Depend, Path, Query
...
async def create_user(ctx: HttpContext, user,          # the validated body
...
The first handler parameter is always the context, exactly as on any other route.
...
The full set is `gt`, `ge`, `lt`, `le`, `multiple_of`, `min_length`, `max_length`, `pattern`, and `strict`. They are covered in depth in [Parameters](/v1.0/guides/validation/parameters/).
...
See [Validation errors](/v1.0/guides/validation/errors/).
...
- **[Parameters](/v1.0/guides/validation/parameters/)**: query strings, headers,
...
- **[Request bodies](/v1.0/guides/validation/request-bodies/)**: `request_model=`,
  ...
  unions.
- **[Forms and file uploads](/v1.0/guides/validation/forms-and-files/)**: urlencoded
  and multipart bodies, the `UploadFile` API, upload validation patterns, and
  the limits worth knowing.
- **[Response models](/v1.0/guides/validation/response-models/)**: enforcing your
  output contract, preventing accidental field leaks, and Pydantic's
  serialization controls in full.
- **[Validation errors](/v1.0/guides/validation/errors/)**: the 422 contract, the
  complete error-type catalog, custom messages, and custom handlers.
- **[Generated documentation](/v1.0/guides/validation/openapi/)**: how declarations
...
[Permissions](/v1.0/guides/permissions/) for the second.
