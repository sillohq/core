...
```python
from sillo import SilloApp, HttpContext, Form, File
...
```python
from sillo import SilloApp, HttpContext, Form, File, Path
...
@app.post("/upload")
async def upload(ctx: HttpContext, document=File(...)):
...
@app.post("/signup")
async def signup(ctx: HttpContext,
...
For the full type catalog and every constraint, see [Parameters](/v1.0/guides/validation/parameters/).
...
@app.post("/tags")
async def add_tags(ctx: HttpContext, tag=Form([], type=List[str])):
    return {"tags": tag}
```
...
@app.post("/avatar")
async def upload(ctx: HttpContext, avatar=File(...)):
...
@app.post("/gallery")
async def gallery(ctx: HttpContext, album=Form(type=str)):
    files = (await ctx.form).getlist("photos")
    for f in files:
...
@app.post("/upload")
async def upload(ctx: HttpContext, caption=Form("", type=str)):
    form = await ctx.form        # served from cache
    files = await ctx.files      # {"avatar": UploadFile}
...
Scan asynchronously through the [queue](/v1.0/guides/work/queue/) and keep files
...
import uuid
from pathlib import Path as FSPath
from sillo import SilloApp, HttpContext, json, File, Form
...
async def upload_document(
    ...
    if document.size and document.size > MAX_BYTES:
        return json({"error": "file too large"}, status_code=413)

    head = await document.read(8)
    await document.seek(0)
    extension = detect(head)
    if extension is None:
        return json({"error": "unsupported file type"}, status_code=415)
    ...
    return json({"id": record.id, "status": "scanning"}, status_code=202)
```
