...
from sillo import SilloApp, HttpContext, Path
...
async def get_user(ctx: HttpContext, user_id=Path(type=int)):
...
@app.get("/users/{user_id}", response_model=UserOut)
async def get_user(ctx: HttpContext, user_id: int = Path(type=int)):
