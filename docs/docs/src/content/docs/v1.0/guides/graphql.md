...
app. The `ctx` context flows through `schema.execute` unchanged.
...
- [Request Inputs](/v1.0/guides/request-inputs/): JSON body parsing used by the
  `POST` handler
- [Authentication](/v1.0/guides/authentication/): protecting the `/graphql` route
- [Middleware](/v1.0/guides/middleware/): ordering auth before the GraphQL route
