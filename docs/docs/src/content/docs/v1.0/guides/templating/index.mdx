...
async def user_context(ctx: HttpContext):
    return {
        "user": await get_current_user(ctx),
        "messages": await get_flash_messages(ctx)
    }
...
async def not_found(ctx: HttpContext, exc):
    return await render(
        "errors/404.html",
        {"path": ctx.url.path},
        status_code=404
...
async def template_error(ctx: HttpContext, exc):
    return await render(
...
the [HTML helpers guide](/v1.0/guides/helpers/html/) for why the built-in
`sanitize_html` is not sufficient for that job.

##  Context injection

`render(..., ctx=ctx)` populates three variables automatically,
each with `setdefault` so your own value always wins:

| Variable | Source | Present when |
|---|---|---|
| `ctx` | the request's context | `ctx=` is passed |
| `url_for` | `ctx.base_app.url_for` | the app exposes it |
| `csrf_token` | `ctx.state.csrf_token` | CSRF middleware has run |

It then merges `ctx.state.template_context`, which is what
...
async def user_context(ctx: HttpContext):
    return {"current_user": await get_current_user(ctx)}
...
Forgetting `ctx=ctx` is the single most common templating bug.
...
**Do not forget `ctx=ctx`.** You lose `csrf_token` and
`url_for`.
