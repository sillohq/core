...
async def login(ctx: HttpContext):
    ...
    ok = await guard.attempt(
        ctx, email=data["email"], password=data["password"]
    )
    if not ok:
...
- [Users & User Models](/v1.0/guides/users/): `SessionUserMixin` wiring
- [JWT](/v1.0/guides/jwt-auth/) · [API Keys](/v1.0/guides/api-keys/)
...
removes the classic [CSRF](/v1.0/guides/csrf/) attack. `Strict` is stronger
...
- [Sessions](/v1.0/guides/sessions/): the session store and its configuration
- [Authentication](/v1.0/guides/authentication/): choosing between strategies
- [CSRF](/v1.0/guides/csrf/): the exposure cookie sessions bring with them
- [Cookies](/v1.0/guides/cookies/): attributes in detail
- [Hashing helpers](/v1.0/guides/helpers/hashing/): verifying passwords
- [Protecting Routes](/v1.0/guides/protecting-routes/): enforcing the session
...
@app.post("/logout")
async def logout(ctx: HttpContext):
    ctx.session.clear()                 # server-side, not just the cookie
...
async def enforce_session_age(ctx: HttpContext, call_next):
    issued = ctx.session.get("issued_at")
...
configuration) are in [Sessions](/v1.0/guides/sessions/). This page covers the
