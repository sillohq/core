---
title: "HTTP Context Internals"
description: "BaseContext, HttpContext, body parsing, form data, file uploads"
---

> **Module**: `sillo.core.http.context` / `sillo.formparser` / `sillo.objects`
...
| File | Responsibility |
|------|---------------|
| `core/sillo/core/http/context.py` | `BaseContext`, `HttpContext`, `cookie_parser`, `ClientDisconnect` |
...
graph TB
    ...
    subgraph "core/sillo/core/http/context.py"
        HTTPCONNECTION["BaseContext"]
        REQUEST["HttpContext"]
...
`HttpContext` inherits from `BaseContext`, adding body consumption, form parsing,
content-type detection, session/user access, and all boolean detectors.

---

## 3. BaseContext: The Base Layer

**Source**: `core/sillo/core/http/context.py`, line 121

`BaseContext` wraps a raw ASGI scope dictionary and provides lazy-cached
property accessors for commonly needed request metadata. It is the shared base
for both `HttpContext` (HTTP) and `WebSocketContext` handlers.
...
`BaseContext` implements `__getitem__`, `__iter__`, and `__len__`, all
delegating directly to `self.scope`. This allows `connection["type"]` style
access:

```python
from sillo import HttpContext, BaseContext

connection = ctx  # HttpContext inherits BaseContext
...
```python
uri = ctx.build_absolute_uri("profile", {"tab": "settings"})
# → "https://example.com/app/profile?tab=settings"
```

---

## 4. HttpContext: The Full HTTP Context

**Source**: `core/sillo/core/http/context.py`, line 583

`HttpContext` extends `BaseContext` with body consumption, form parsing, content
...
async with ctx.form_data as form:
    name = form.get("name")
```
...
**Source**: `core/sillo/core/http/context.py`, line 54
...
**Source**: `core/sillo/core/http/context.py`, line 100

```python
class ClientDisconnect(Exception):
    """Exception raised when the HTTP client disconnects during ctx processing."""
```

This is a bare exception class with no additional payload. It is raised by
`HttpContext.stream()` when the ASGI server delivers an `http.disconnect` message
...
On success, the caller (`HttpContext.close()` or the `AwaitableOrContextManager`
...
|----------|-----------|-------------|
| `BaseContext.url` | `_url` | `URL(scope=self.scope)` |
| `BaseContext.base_url` | `_base_url` | Modified scope → `URL(scope=...)` |
| `BaseContext.headers` | `_headers` | `Headers(scope=self.scope)` |
| `BaseContext.query_params` | `_query_params` | `QueryParams(scope["query_string"])` |
| `BaseContext.cookies` | `_cookies` | `cookie_parser(header)` |
| `BaseContext.state` | `_state` | `State(scope["state"])` |
...
**Source**: `core/sillo/core/http/context.py`, lines 1190 to 1343 and 1377 to
1456
...
async def get_user(ctx: HttpContext):
    ...
    # Query parameters
    include_posts = ctx.query_params.get("include_posts", "false")
...
@app.post("/api/data")
async def receive_json(ctx: HttpContext):
    if not ctx.is_json:
        return {"error": "Expected JSON"}, 415

    data = await ctx.json
    text = await ctx.text  # Also available
...
@app.post("/stream")
async def stream_body(ctx: HttpContext):
    total = 0
...
async def long_upload(ctx: HttpContext):
    chunks = []
    async for chunk in ctx.stream():
        chunks.append(chunk)
...
async def timing_middleware(ctx: HttpContext, call_next):
    ...
    elapsed = time.monotonic() - ctx.state.start_time
...
@app.get("/redirect")
async def redirect_example(ctx: HttpContext):
    callback_url = ctx.build_absolute_uri(
...
@app.get("/data")
async def content_negotiation(ctx: HttpContext):
    if ctx.accepts_json:
        return {"format": "json"}
    elif ctx.accepts_html:
        return "<h1>Data</h1>"
...
ctx = HttpContext(scope, receive=mock_receive)
body = await ctx.body  # b'{"key": "value"}'
```
...
ctx = HttpContext(scope, receive=disconnect_receive)
...
headers1 = ctx.headers
# Second access returns cached instance
headers2 = ctx.headers
...
|------|-------|-------------|
| `core/sillo/core/http/context.py` | — | `BaseContext`, `HttpContext`, `cookie_parser`, `ClientDisconnect`, `empty_receive`, `empty_send` |
...
- `HttpContext` objects are designed for a single-request lifecycle: one context
