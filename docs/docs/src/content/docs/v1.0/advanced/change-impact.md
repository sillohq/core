...
| Change | Impact |
|--------|--------|
| `dispatch` signature | All middleware subclasses break |
| `call_next` taking arguments | Every middleware body breaks |
| `__call__` forwarding to `dispatch` | Subclasses overriding `__call__` break |
...
`dispatch(ctx, call_next)` is a **core contract**, and the one thing every
middleware in and outside the framework is written against. Changing its shape
is a major-version change; there is no way to do it compatibly, because the
...
| Dependent | How |
|-----------|-----|
| Every handler | Receives `HttpContext`, returns a response |
| Every middleware | `dispatch(ctx, call_next)` |
| `useAuth` | Reads `ctx.user`, `ctx.session` |
| Test client | Constructs `HttpContext` objects |
| Form parsing | `await ctx.form`, `await ctx.files` |
| DI system | Extracts parameters from the context |
...
`HttpContext` and `BaseResponse` are the two most widely used classes.  Any change
