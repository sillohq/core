...
mail = app.install(Mail(MailConfig.for_gmail("user@gmail.com", "app-password")))
```
</TabItem>
...
mail = setup_mail(app, MailConfig.for_gmail("user@gmail.com", "app-password"))
```
</TabItem>
</Tabs>

`Mail` is the recommended declarative form. Both forms put the same client in
`app.state["mail_client"]`, register it for process-wide lookup, and connect
and stop it with the application lifecycle.
...
`Mail` and `setup_mail` register the client with `sillo._internals.registry`, the same
...
```python
from sillo.mail import Mail, send_email, MailConfig

# At startup
config = MailConfig.for_gmail("user@gmail.com", "app-password")
app.install(Mail(config))
