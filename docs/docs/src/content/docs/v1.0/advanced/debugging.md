...
```python
# Middleware authenticates (sets ctx.user)
...
# RIGHT: ctx.base_app is the root app
db = ctx.base_app.state["record"]
```

**File**: `core/sillo/core/http/context.py`, `BaseContext.app`,
`BaseContext.base_app`
...
graph TD
    ...
    B -->|No| C{Cookie set on the returned response?}
    C -->|Yes| D[set_cookie writes a Set-Cookie header]
...
### 10.1 Setting cookies on the response you return

**Problem**: A cookie set in a handler doesn't appear in the response.

**Diagnosis**: `set_cookie` is a method on a response, and the response is the
value the handler returns. Setting a cookie on one object and returning another
sends the one without the cookie.

```python
# WRONG: the cookie goes on a response that is then thrown away
...
return json({"ok": True}).set_cookie("session", "abc", secure=False)
```
