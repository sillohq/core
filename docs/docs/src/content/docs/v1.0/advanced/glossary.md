...
**Related:** `Route`, `WebSocketContext`, `Channel`
...
**Module:** `sillo.core.http.context`

ASGI request wrapper.  Inherits from `BaseContext`.
...
**Inherited from `BaseContext`:** `app`, `base_app`, `url`, `base_url`,
...
**Differs from Django's `HttpRequest`:** Sillo's `HttpContext` is async-first.
`body`, `json()`, `form()`, `files()` are all `async`.  Django's request is
sync with lazy attribute access.

**Differs from Starlette's `HttpContext`:** Sillo adds `user`, `session`,
`is_ajax`, `accepts_html` properties, and `valid()` for DI validation.

**Related:** `BaseResponse`, `BaseContext`, `Session`, `UserProtocol`
...
**Related:** `sillo.responses`, `HttpContext`

---

### sillo.responses

**Module:** `sillo.responses`

One free function per response shape, each returning a `BaseResponse` subclass.
Every name is also re-exported from the root `sillo` package.

**Builders:** `json`, `text`, `html`, `xml`, `raw`, `empty`, `created`,
`accepted`, `no_content`, `redirect`, `permanent_redirect`, `see_other`,
`temporary_redirect`, `file`, `download`, `stream`, `ndjson`, `sse`,
`paginate`, `apaginate`, `abort`, `not_found`.

`paginate` and `apaginate` take the context first. `abort` and `not_found`
raise rather than returning.

**Differs from Django's `HttpResponse`:** these are functions, not classes. The
class each one returns is the thing you can subclass.
...
**Methods:**
- `__call__(ctx, call_next)`: entry point; forwards to `dispatch`
- `dispatch(ctx, call_next)`: the one hook subclasses override

**Key mechanism:** none. Whatever `dispatch` returns is the response. Awaiting
`call_next()` runs the rest of the chain; returning without awaiting it
short-circuits.

**Differs from Django's middleware:** Django splits the two phases into
separate hooks. Sillo has one, and the phases are the two sides of the
`await` — which is what lets a single `try` span both.
...
**Related:** `SilloApp`, `HttpContext`, `BaseResponse`, `AuthenticationMiddleware`
...
async def get_db(ctx: HttpContext) -> Database:
    ...
...
**Methods:** `dispatch(ctx, call_next)`, `_persist(ctx, response)`.
...
**Related:** `BaseMiddleware`, `BaseResponse`
...
**Related:** `HttpContext`, `accepts_html`
