...
def login(ctx: HttpContext, user, session_key="user", identifier="id"):
    assert "session" in ctx.scope, "No Session Middleware Installed"
    if ctx.session.get(session_key):
        del ctx.session[session_key]
    ctx.session[session_key] = {
        identifier: user.identity,
        "display_name": user.display_name,
    }

def logout(ctx: HttpContext, session_key="user"):
    assert "session" in ctx.scope, "No Session Middleware Installed"
    del ctx.session[session_key]
```
...
success = await guard.attempt(ctx, email="user@example.com", password="secret123")

# 3. Internally:
#    a. user_model.objects.get_by_email("user@example.com") → User instance
#    b. user.check_password("secret123") → True
#    c. login(ctx, user) → stores {id: "42", display_name: "alice"} in session
...
```python
# 1. Subsequent ctx includes session cookie
GET /profile
Cookie: session_id=<signed_session_data>

# 2. SessionMiddleware loads session from cookie → ctx.session

# 3. SessionAuthBackend.authenticate(ctx):
#    a. Reads ctx.session.get("user") → {"id": "42", "display_name": "alice"}
...
#    b. Sets ctx.scope["user"] = user
#    c. Sets ctx.scope["auth"] = "sessionCookie"
#    d. Sets ctx.scope["auth_scheme"] = "sessionCookie"
```
...
# 2. JWTAuthBackend.authenticate(ctx):
...
```python
# In AuthenticationMiddleware.dispatch:
...
async def test_jwt_backend_valid_token():
    ...
    ctx = MockRequest(headers={"Authorization": f"Bearer {token}"})
...
async def test_session_backend_with_session():
    ...
    ctx = MockRequest(scope={"session": {"user": {"id": "42", "display_name": "alice"}}})
    result = await backend.authenticate(ctx)
...
async def test_apikey_backend_stateless():
    ...
    ctx = MockRequest(headers={"X-API-Key": "my-secret-key"})
    result = await backend.authenticate(ctx)
