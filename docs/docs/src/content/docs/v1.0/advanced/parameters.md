...
async def handler(ctx: HttpContext, page=Query(1, type=int, ge=1)):
    ...
```
...
```python
from sillo import HttpContext, Query, Header, Cookie, Path, Form, File

@app.post("/teams/{team_id}/upload",
          request_model=TeamMeta,
          response_model=UploadResult)
async def upload_team_avatar(
    ctx: HttpContext, response,                    # Framework-injected
...
@app.get("/items")
async def list_items(ctx: HttpContext, page=page_marker):
    ...

@app.get("/users")
async def list_users(ctx: HttpContext, page=page_marker):
    ...
```
