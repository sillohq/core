...
- Middleware must be able to act on the request on the way in and the response on the way out.
...
async def get_user(ctx: HttpContext) -> User:
    ...
...
profile = await exchange(provider, ctx)
```
