...
def _build_request_chain(self) -> None:
    ...
    # response on the way out, so neither needs a ctx/response bridge.
...
2. **HTTP scopes**: Creates a `_CachedRequest` and an
...
`_CachedRequest` extends `HttpContext` with body caching for dispatch middleware:
...
MiddlewareType = Callable[
    ...
    Awaitable[BaseResponse | StreamingResponse],
]

# Where:
RequestResponseEndpoint = Callable[[], Awaitable[BaseResponse | StreamingResponse]]
```
...
async def logging_middleware(ctx: HttpContext, call_next):
    ...
    print(f"{ctx.method} {ctx.url.path} took {duration:.3f}s")
    return result
...
#    - Server calls app.__call__ for each HTTP ctx
#    - handle_request() builds middleware chain
#    - The request flows through: SEM → CORS → Auth → Logging → Exception → Router → Handler
