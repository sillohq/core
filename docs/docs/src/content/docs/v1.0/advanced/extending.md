...
There is nothing internal to it. `__call__` forwards to `dispatch`, and
whatever `dispatch` returns becomes the response — either the one it got back
from `call_next()`, possibly modified, or one it built itself to short-circuit.
...
### 3. The `BaseMiddleware` signature
...
Call `call_next` at most once. Awaiting it twice runs the rest of the chain
twice against a body that has already been consumed.
