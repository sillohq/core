...
```python
from sillo.storage import StorageConfig, setup_storage
...
Both forms put the same `Storage` in `app.state["storage"]`, register it for
global lookup, mount a serving route when configured, and close drivers during
shutdown.
...
Both are registered by `StorageInstallable` or `setup_storage`, alongside the existing
