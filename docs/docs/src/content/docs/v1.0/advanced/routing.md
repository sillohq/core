...
async def get_route_handler(self, ctx: HttpContext, **kwargs):
    cleanup_callbacks = []
    injected = {}
    dependency_cache = {}
    ...
    for rd in self._router_dependants:
        sub_values = await solve_dependencies(rd, ctx, dependency_cache, cleanup_callbacks)
    ...
    handler_values = await solve_dependencies(self.dependant, ctx, dependency_cache, cleanup_callbacks)
    injected.update(handler_values)

    # 3. Validate ctx body if request_model is set
    ...
    try:
        ...
        return await run_in_threadpool(self.handler, ctx, response, **kwargs, **injected)
    finally:
...
1. Skip the first parameter (the context).
...
5. **Handler invocation**: receives the context, path params (`**kwargs`), and injected DI values (`**injected`).
...
The auth gate runs **after** DI and body validation but **before** the handler. It receives the `HttpContext` object and should raise an exception (e.g., `HTTPException(401)`) if authentication fails.
...
```python
if isinstance(func_result, (BaseResponse, BaseResponse)):
...
**Key rule**: If the handler returns a `BaseResponse`, the response model is **not applied**. The handler has full control.
...
async def get_user(ctx: HttpContext, id: int):
    return {"user_id": id}
