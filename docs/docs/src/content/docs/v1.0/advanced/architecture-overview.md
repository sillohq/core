...
templates, permissions, and validation. These modules depend on the
core engine.
...
| `core/http/` | 2 | `HttpContext`, `BaseResponse`, `StreamingResponse` |
...
2. Builds an `HttpContext` from the ASGI scope
...
```python
async def solve_dependencies(dependant, ctx, ...) -> dict[str, Any]:
    values = {}
    validated = await resolve_validated_params(dependant, ctx)
...
ctx.scope["base_app"]      # Root SilloApp (preserved across mounts)
ctx.scope["global_state"]  # app.state dict
ctx.scope["root_path"]     # ASGI mount prefix
ctx.scope["path"]          # Request path
ctx.scope["method"]        # HTTP method
ctx.scope["headers"]       # Raw header byte tuples
ctx.scope["query_string"]  # Raw query bytes
ctx.scope["path_params"]   # Route parameters (injected by Router)
```
...
```python
ctx.scope["global_state"]["db_pool"]
# or through the State object wrapper:
ctx.state  # State object wrapping per-request state
```
...
| Context/Response | `core/http/` | `HttpContext`, `BaseResponse` |
