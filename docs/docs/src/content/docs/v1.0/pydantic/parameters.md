...
`minimum: 1` in your [OpenAPI document](/v1.0/pydantic/openapi/), so the validation
...
`strict` is for a [JSON body](/v1.0/pydantic/request-models/), where the client
controls the types.
:::
...
[error paths](/v1.0/pydantic/errors/#loc); `page_size` is the Python name.
...
@app.get("/posts/{post_id:int}")
async def get_post(ctx: HttpContext, post_id=Path(type=int)):
    ...
```
...
```python
from sillo import HttpContext, File, Form
...
filename as a path. See [File uploads](/v1.0/guides/file-upload/).
:::

Do not mix `Form`/`File` with [`request_model`](/v1.0/pydantic/request-models/). A
...
string from a malformed body. See [Validation errors](/v1.0/pydantic/errors/).
...
```python
from sillo import HttpContext, Depend, Query
...
@app.get("/posts")
async def list_posts(ctx: HttpContext, paging=Depend(pagination)):
    ...
```
...
- [Request models](/v1.0/pydantic/request-models/): the JSON body
- [Fields](/v1.0/pydantic/fields/): the same constraints on model fields
- [Request parameters](/v1.0/guides/request-parameters/): the wider guide
