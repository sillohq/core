...
async def get_user(ctx: HttpContext, id=Path(type=int)):
...
[every field the model has](/v1.0/orm/models/#serialisation) and quietly starts
...
For an envelope, use a [generic model](/v1.0/pydantic/models/#generic-models)
instead:
...
[aliases](/v1.0/pydantic/fields/#aliases) emits the alias without further
...
not transfer every post body. See [Patterns](/v1.0/pydantic/patterns/).
