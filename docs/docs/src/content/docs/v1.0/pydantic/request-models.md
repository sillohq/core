...
document](/v1.0/pydantic/openapi/) describe the request without introspecting the
...
async def create_post(ctx: HttpContext, payload): ...
async def create_post(ctx: HttpContext, data): ...
...
async def create_post(
    ctx: HttpContext, response, payload,
...
async def create_post(ctx: HttpContext):
    payload = ctx.validated_data
```

## What happens on failure

A body that does not validate never reaches the handler. The client gets
[422 with every failure listed](/v1.0/pydantic/errors/):
...
[Configuration](/v1.0/pydantic/config/#defaults-worth-adopting).

## What the model must not do

A request model validates **shape**. It cannot validate facts about your
database, because [Pydantic validators are synchronous](/v1.0/pydantic/validators/#where-validation-belongs):
...
which the [exception handlers](/v1.0/orm/exceptions/) already turn into a 409. That
...
The same reasoning as [mass assignment](/v1.0/orm/mass-assignment/) on the ORM side,
...
to make it optional. See [Types](/v1.0/pydantic/types/#unions-and-optionals).
...
See [Parameters](/v1.0/pydantic/parameters/#form-and-file).
...
Both go straight into the [OpenAPI document](/v1.0/pydantic/openapi/). A single
