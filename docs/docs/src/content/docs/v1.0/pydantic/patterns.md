...
async def list_posts(ctx: HttpContext, page=Query(1, type=int, ge=1)):
...
own [named schema](/v1.0/pydantic/openapi/#naming-and-collisions)
...
@app.get("/posts", response_model=Page[PostSummary])
async def list_posts(ctx: HttpContext, filters=Depend(post_filters)):
    query = Post.all()
    if filters.status:
...
that uses it. See [Parameters](/v1.0/pydantic/parameters/#dependencies) and
[building a Q up conditionally](/v1.0/orm/filtering/#building-one-up).
...
[`SecretStr`](/v1.0/pydantic/types/#pydantics-own-types) keeps the key out of
...
A [model validator](/v1.0/pydantic/validators/#model_validator) is the right place.
...
[customising the 422](/v1.0/pydantic/errors/#customising-the-response).
...
[constraint](/v1.0/orm/meta/#constraints) and catch the error.
