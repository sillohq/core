...
description: Timeout middleware and utilities for the sillo web framework.
---

# Timeout Middleware

Timeout middleware and utilities for sillo web framework, providing flexible request timeout handling, duration tracking, and timeout-based control flow.
...
```python
import asyncpg
from sillo_contrib.timeout import timeout_after
...
```python
from sillo_contrib.timeout import (
...
async def timeout_debug(request, response, call_next):
    from sillo_contrib.timeout import get_timeout_from_request
