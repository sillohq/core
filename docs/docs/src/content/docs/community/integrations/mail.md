...
description: The sillo Mail contrib module provides a comprehensive email sending solution.
---

# Mail Integration

The sillo Mail contrib module provides a comprehensive email sending solution with SMTP support, template integration, and background task processing.

## Overview

sillo Mail is a powerful and easy-to-use email sending solution for sillo applications that includes:

- **SMTP Support**: Full SMTP configuration with TLS/SSL support
- **Template Integration**: Jinja2-based HTML email templates
- **Background Tasks**: Async email sending with sillo-contrib tasks
- **Dependency Injection**: Easy integration with sillo applications
...
```python
from sillo_contrib.mail import MailClient, MailConfig
...
```python
from sillo_contrib.mail import EmailMessage
...
async def send_advanced_email():
    ...
    try:
        ...
        message.add_header("X-Mailer", "sillo Mail")
...
## Integration with sillo Applications

While direct MailClient usage is simple, integrating with sillo applications provides additional benefits like automatic lifecycle management and dependency injection.
...
```python
from sillo_contrib.mail import get_mail_from_request
...
The mail module integrates seamlessly with the sillo-contrib tasks module for async email sending:
...
This comprehensive mail integration provides everything you need to add professional email functionality to your sillo applications with ease.
