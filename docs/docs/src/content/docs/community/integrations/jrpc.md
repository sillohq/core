---
title: JSON-RPC Integration
description: Lightweight helpers for running JSON-RPC 2.0 over HTTP with sillo.
---

# JSON-RPC Integration

Lightweight helpers for running JSON-RPC 2.0 over HTTP with sillo.
...
The JSON-RPC contrib is included with the main sillo_contrib package, making it easy to get started with remote procedure calls.
...
Creating a JSON-RPC server with sillo is straightforward. The server automatically handles request parsing, method dispatch, and response formatting.
...
def main():
    ...
    # Mount the JSON-RPC plugin to your sillo app
    # This creates an HTTP endpoint that accepts JSON-RPC requests
    # The plugin integrates with sillo's routing and middleware system
    JsonRpcPlugin(app, {"path_prefix": "/rpc"})
...
# client.py
import asyncio
from sillo_contrib.jrpc.client import JsonRpcClient
...
```python
from sillo_contrib.jrpc.exceptions import JsonRpcError, JsonRpcClientError
import asyncio
import logging
...
## Integration with sillo Framework

### Middleware Integration

JSON-RPC seamlessly integrates with sillo's middleware system, allowing you to add cross-cutting concerns like logging, authentication, and monitoring.
...
if __name__ == "__main__":
    print(" Starting sillo JSON-RPC server with middleware...")
