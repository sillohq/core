...
queue — and it is a lot to meet at once. If you have not written a Sillo
route yet, [Installation](/guides/installation/) builds a single-file
application from nothing in about a minute, and
[Routing](/guides/routing/) → [Handlers](/guides/handlers/) →
[Validation](/guides/validation/) is enough to build something real.
...
`uv run` under uv, or `poetry run` under Poetry. With an activated `venv`,
type them as written.
:::

The starter ships a `uv.lock`, so `uv sync` reproduces the exact set of
versions its CI tested. Poetry and PDM install it too — the `pyproject.toml`
is standard PEP 621 — but they resolve fresh rather than reading that lock.
...
but nothing here requires it — `venv` and `pip` ship with Python and do the
same job.
...
- [http://localhost:8000/docs](http://localhost:8000/docs) — the generated OpenAPI documentation
- [http://localhost:8000/admin/](http://localhost:8000/admin/) — the admin panel, note the trailing slash
