...
Register the handlers in any application where handlers query the
...
:::caution[`IntegrityError` and `ValidationError` messages describe your schema]
...
For an internal API this is helpful. For a public one it hands an
attacker your table and column names, and — with a unique constraint on
email — a user-enumeration oracle: a signup that returns 409 confirms
...
:::caution[A handler for `OperationalError` catches far more than connection failures]
If you register your own `OperationalError` handler — reasonably assuming
it means "the database is unreachable" — it will also receive every
`DoesNotExist` and every `IntegrityError` for which no more specific
handler is registered. A row that simply does not exist would be reported
to the client as a database outage.
...
The four handlers cover the common cases, not every database error.
...
:::danger[Every field is required by default, including `id` and the timestamps]
The generator decides required-ness with `is_required = not field.null and not field.pk`,
then writes `Field(default=...)` for the non-required branch — and
`Ellipsis` as a Pydantic default means *required*. So primary keys come
out required after all, and so do `created_at` and `updated_at`, which
are non-null and auto-populated.
...
Subclassed fields replace the generated ones by name, so you only restate
what the generator got wrong. This is the pattern to reach for whenever a
model has money, dates, emails, URLs, or enums in it — which is most
models.
...
`optional_fields` has to list every field you want optional — there is no
...
`register_db_exception_handlers` costs nothing at request time — handler
...
- [Models & Mixins](/guides/record/models/) — `update_from_dict` and mass-assignment safety
- [Transactions & Factories](/guides/record/transactions-factories/) — where `IntegrityError` usually comes from
- [Pagination](/guides/record/pagination/) — the other place errors need mapping to 400
- [Error Handling](/guides/error-handling/) — sillo's exception middleware in general
- [Request Inputs](/guides/request-inputs/) — how `request_model` validation fits the request lifecycle
