...
A model that declares neither `fillable` nor `guarded` accepts any key
matching a field — including `id`, `created_at`, `deleted_at`, and
`is_admin` — so passing an unvalidated request body straight in is a
