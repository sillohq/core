...
<Aside type="danger" title="The &quot;encrypted&quot; cast is not encryption">
...
Note that encrypted columns cannot be filtered, sorted, or indexed by value.
Every `WHERE ssn = ...` becomes a full scan and decrypt. If you need lookups,
store a separate HMAC of the value and query that. See [Crypto
helpers](/guides/helpers/crypto/).
