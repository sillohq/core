...
**[HTML](/guides/helpers/html/). `sanitize_html` is not safe for untrusted
...
**[Network](/guides/helpers/network/): `get_client_ip` trusts every header by
...
**[Hashing](/guides/helpers/hashing/): never store a password with a digest.**
...
**[Crypto](/guides/helpers/crypto/). `unsign_value(max_age=...)` does
...
signed payload, or use [JWT](/guides/helpers/jwt/).
...
or a [background task](/guides/work/background/).
...
- [Authentication](/guides/authentication/): the full auth stack built on
  hashing and JWT
...
- [Validation](/guides/validation/): input validation, which these helpers do
  not do
