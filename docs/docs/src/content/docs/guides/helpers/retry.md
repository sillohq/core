...
:::caution[The original exception is wrapped, not re-raised]
...
:::danger[The default retries bugs in your own code]
...
For HTTP work, the status code matters more than the exception type. Retry 429, 502, 503, and 504. Do not retry 400, 401, 403, 404, or 422 — the request is wrong and repeating it verbatim cannot help.
:::
...
:::caution[A name collision between your kwargs and the retry kwargs is silent]
...
:::danger[Do not use long backoff inside a request handler]
...
Inside a request, keep the total retry budget under your own timeout — two attempts with a short delay at most:
...
async def dashboard(request, response):
    ...
    return await response.template("dashboard.html", {"stats": stats})
```

Long, patient retries belong in [Background Tasks](/guides/work/background/) and the [Job Queue](/guides/work/queue/), where nobody is waiting on the socket. If an operation can afford a minute of retries, it can afford to be a job.
:::
...
- [Background Tasks](/guides/work/background/) — where patient retries belong
- [Job Queue](/guides/work/queue/) — durable retries that survive a restart
- [HTTP Client](/guides/http/client/) — has its own retry configuration
- [Error Handling](/guides/error-handling/) — turning exhausted retries into responses
- [Async Helpers](/guides/helpers/async/) — sync and async detection, which the decorator uses
