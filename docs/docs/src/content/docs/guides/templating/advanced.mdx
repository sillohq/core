...
and key building) documented in [Cache](/guides/cache/). The in-memory example
...
- [Templating](/guides/templating/): engine setup, autoescaping, and
  configuration
- [Cache](/guides/cache/): the real cache module, for fragment and page caching
- [HTML helpers](/guides/helpers/html/): escaping and why `sanitize_html` is
  not enough
- [CSRF](/guides/csrf/): where `csrf_token` comes from
- [Error Handling](/guides/error-handling/): registering the error handlers
  shown above
