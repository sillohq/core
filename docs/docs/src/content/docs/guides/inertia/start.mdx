...
Install it once if you have not already — `uv tool install sillo-start`, or
`pip install sillo-start`. See [Creating a Project](/guides/start/#install-sillo-start).
...
console full of failed module requests. With only `npm run dev` you get
nothing at all — Vite serves modules, not your application.
