...
The JSON branch accepts either frame type — it decodes a binary frame as
UTF-8 before parsing — so a client that sends JSON over binary frames
works without configuration.

:::caution[Malformed JSON kills the connection]
...
If clients might send garbage — anything driven by a browser
extension, a flaky mobile client, or a hand-written reconnect loop —
leave `encoding` unset and parse in your handler, where you can reply
...
Every consumer instance gets `self.channel` — a `Channel` wrapping the
...
:::danger[`leave_group` can raise `UnboundLocalError`]
...
This is reachable from ordinary code. A consumer whose `on_connect`
failed before joining, or one that leaves a group twice, will hit it —
and because `leave_group` is normally called from `on_disconnect`, the
error surfaces during cleanup where it is least expected.
...
`as_route` returns a `WebsocketRoute` whose handler instantiates the
...
One consumer instance and one asyncio task per connection. The instance
is cheap; what it holds is not — a loaded user object per connection, at
ten thousand connections, is ten thousand user objects.
