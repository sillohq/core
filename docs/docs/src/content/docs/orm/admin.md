...
| `auth_backend` | A full [auth backend](/orm/admin-permissions/#authentication-backends). Overrides `user_model`. |
...
Any [`UserBaseModel`](/guides/users/) subclass works.
...
([Permissions](/orm/admin-permissions/)), and consider disabling the admin
