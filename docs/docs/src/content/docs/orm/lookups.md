...
[relation to traverse](/orm/relationships/#spanning-relations-in-queries). With
...
[search box](/orm/admin-customising/#search) is `icontains`, and is meant for
...
[`.sql()`](/orm/queryset/#inspecting) before relying on it.
...
[`JSONField`](/orm/field-reference/#json) has its own set, not the ones above:
...
appears twice. [`distinct()`](/orm/values/#distinct) collapses them.
...
what lets it see the aggregate. See [Aggregation](/orm/aggregation/).
