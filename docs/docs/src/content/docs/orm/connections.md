...
[`setup_record`](/orm/setup/) registers one connection named `default`.
...
`ensure_context` middleware [`setup_record` registers](/orm/setup/#ensure_context)
...
Use [background tasks](/guides/work/background/) or a
[queued job](/guides/work/queue/), both of which carry it. A job also survives
...
The `prefix` on [`from_env`](/orm/configuration/#fields) is what keeps the two
...
See [Transactions](/orm/transactions/#multiple-connections).
...
[standalone console](/cli/standalone-consoles/), where nothing has.
...
[migration commands](/cli/standalone-consoles/#record_commands):
...
[`db_constraint=False`](/orm/relationships/#db_constraintfalse) foreign key is
