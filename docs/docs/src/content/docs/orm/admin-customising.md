...
[`fillable` over `guarded`](/orm/mass-assignment/#which-to-reach-for).
...
It is set-based SQL: no [events](/orm/events/), no
[casts](/orm/casting/), no [validation](/orm/mixins/#validatesbeforesavemixin),
...
[`CascadesDeletesMixin`](/orm/mixins/#cascadesdeletesmixin) still cascades.
...
Both are recorded in the [activity log](/orm/admin/#the-activity-log) with the
