...
Related: the [`json` cast](/orm/casting/) does the same conversion over a
...
| A value derived from other fields | A [model event](/orm/events/) |
| A value that must always hold | A [check constraint](/orm/meta/#constraints) |
