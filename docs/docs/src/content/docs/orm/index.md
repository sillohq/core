...
- [Configuration](/orm/configuration/) — `DatabaseConfig`, URLs, backends, pooling
...
that is true, the page says so plainly — see for example the
[`encrypted` cast](/orm/casting/#the-encrypted-cast-is-not-encryption).
