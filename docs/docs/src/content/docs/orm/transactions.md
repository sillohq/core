...
- [`bulk_create` and friends](/orm/bulk/) write in batches. Without a
  transaction, a failure halfway leaves earlier batches committed.
- [`after_*` model events](/orm/events/#failure-semantics) fire inside the
  transaction when the save is inside one, so a failing hook rolls the write
  back.
- [`ensure_context`](/orm/setup/#ensure_context) is what makes the connection
