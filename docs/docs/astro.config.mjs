...
/**
 * The old, unversioned URL for every page in the default version, pointed at
 * its new home.
 *
 * Astro will not take a wildcard destination — `/guides/[...slug]` has to
 * resolve to a route that exists — so the map is built from the content tree
 * rather than written out or globbed with a pattern. Vercel does the same
 * thing with two wildcards at the edge (see ../vercel.json); these exist so
 * `astro dev` and `astro preview` behave the same way locally, which is the
 * only place the old links are ever followed without the edge in front.
 */
function unversionedRedirects() {
    const root = new URL(`./src/content/docs/${DEFAULT_VERSION}/`, import.meta.url);
    ...
    return Object.fromEntries(
        entries
            ...
            .map((entry) => {
                // Dirent.parentPath is absolute; slice back to the manual segment.
                const dir = relative(fileURLToPath(root), entry.parentPath);
                const slug = entry.name.replace(/\.mdx?$/, '');
                // `index` is the directory itself, as Starlight routes it.
                const path = [dir, slug === 'index' ? '' : slug].filter(Boolean).join('/');
                return [
                    `/${path}/`,
...
/*
 * The sidebar, written once and generated per version.
 *
 * These are the groups the theme's Sidebar filters down to on each page: it
 * keeps only the ones whose links match the current URL's prefix, which is
 * what gives every manual its own sidebar out of one flat array. Versioning
 * did not change that, it only lengthened the prefix from `/guides/` to
 * `/v1.0/guides/` — see src/components/Sidebar.astro.
 *
 * Links are written unversioned here and prefixed by `sidebarFor`, so adding a
 * page means adding one line rather than one line per version.
 */
const MANUAL_GROUPS = [
    // -- Sillo Start ------------------------------------------
    {
    ...
    },
    {
        ...
        items: [
            ...
            { label: 'Storage', link: '/advanced/storage/' },
...
export default defineConfig({
    ...
    //
    // Every manual moved down one segment when the docs were versioned, so the
    // unversioned paths that are linked from the marketing site and indexed by
    // search engines all point at the default version now. These are 302s, not
    // 301s: which version `/guides/routing/` means changes when v1.0 ships,
    // and a cached permanent redirect would outlive that.
    redirects: {
        '/': {
            status: 302,
            destination: `/${DEFAULT_VERSION}/guides/introduction/`,
        },
        '/guides/getting-started/': {
            status: 302,
            destination: `/${DEFAULT_VERSION}/guides/introduction/`,
        },
        ...unversionedRedirects(),
    },
    ...
    integrations: [
        starlight({
            ...
            /*
             * One sidebar per manual per version — see MANUAL_GROUPS above and
