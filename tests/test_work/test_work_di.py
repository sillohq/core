...
def test_scheduler_injection():
    ...
    @app.get("/scheduler")
    async def handler(ctx: HttpContext, s=Depend(scheduler, get_context=True)):
        assert s is sched
        return json({"ok": True})
...
def test_queue_connection_injection():
    ...
    @app.get("/queue")
    async def handler(ctx: HttpContext, c=Depend(queue_connection, get_context=True)):
        assert c is conn
        return json({"ok": True})
...
def test_events_injection():
    ...
    @app.get("/events")
    async def handler(ctx: HttpContext, d=Depend(events, get_context=True)):
...
def test_provider_raises_when_not_setup():
    ...
    @app.get("/test")
    async def handler(ctx: HttpContext, x=Depend(provider, get_context=True)):
        assert x is None
        return json({"ok": True})
