...
def test_int_default_still_coerces(app, client):
    ...
    @app.get("/legacy")
    async def handler(request, response, page=Query(1)):
        return response.json({"page": page, "type": type(page).__name__})
...
def test_missing_param_without_default_is_none(app, client):
    ...
    @app.get("/legacy")
    async def handler(request, response, q=Query()):
...
def test_no_default_returns_raw_string(app, client):
    ...
    @app.get("/legacy")
    async def handler(request, response, q=Query()):
        return response.json({"type": type(q).__name__})
...
def test_required_missing_is_500(app):
    ...
    @app.get("/legacy")
    async def handler(request, response, q=Query(required=True)):
        return response.json({"q": q})
...
def test_list_default_comma_splits(app, client):
    ...
    @app.get("/legacy")
    async def handler(request, response, tags=Query([])):
...
def test_bool_default_coerces_yes(app, client):
    ...
    @app.get("/legacy")
    async def handler(request, response, active=Query(False)):
...
def test_header_and_cookie_defaults(app, client):
    ...
    @app.get("/legacy")
    async def handler(request, response, x_api_key=Header(), theme=Cookie("dark")):
        return response.json({"key": x_api_key, "theme": theme})
...
def test_metadata_only_does_not_change_behavior(app, client):
    ...
    @app.get("/legacy")
    async def handler(request, response, q=Query(description="a query")):
        return response.json({"q": q})
