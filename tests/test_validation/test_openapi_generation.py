...
def test_constraints_appear_in_schema(app, client):
    @app.get("/items")
    async def handler(request, response, page=Query(1, type=int, ge=1, le=99)):
...
def test_description_appears_in_schema(app, client):
    @app.get("/items")
    async def handler(request, response, q=Query(type=str, description="Search text")):
        return response.json({})
...
def test_urlencoded_body_when_no_file(app, client):
    @app.post("/form")
    async def handler(request, response, title=Form(type=str)):
        return response.json({})
...
def test_legacy_param_schema_is_unchanged(app, client):
    ...
    @app.get("/items")
    async def handler(request, response, page=Query(1), limit=Query(10)):
        return response.json({})
...
def test_dependency_params_are_documented(app, client):
    ...
    @app.get("/items")
    async def handler(request, response, p=Depend(pager)):
        return response.json({})
