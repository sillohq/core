...
class QuerySet(_TortoiseModel._meta.default_connection.__class__.QuerySet if False else Any):
...
class Model(_TortoiseModel):
    ...
    def to_dict(self, *, exclude: Optional[List[str]] = None, include: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    @classmethod
    async def bulk_create(cls: Type[T], items: List[Dict[str, Any]], batch_size: int = 100) -> List[T]:
