...
class TimeoutMiddleware:
    ...
    async def before_enqueue(self, task: Task) -> None: pass

    async def before_execute(self, task: Task) -> None:
        if not getattr(task, "timeout", None):
            object.__setattr__(task, "timeout", self.timeout)

    async def after_execute(self, result: TaskResult) -> None: pass

    async def on_error(self, task: Task, error: Exception) -> None: pass
...
class RateLimitMiddleware:
    ...
    async def before_execute(self, task: Task) -> None:
        ...
        self._tokens = min(float(self.burst), self._tokens + elapsed * self.max_per_second)
...
class LoggingMiddleware:
    ...
    async def after_execute(self, result: TaskResult) -> None:
        logger.log(self.level, f"DONE    {result.name} [{result.task_id[:8]}] ok={result.ok} ({result.duration_ms}ms)")

    async def on_error(self, task: Task, error: Exception) -> None:
        logger.log(logging.WARNING, f"ERROR   {task.name} [{task.id[:8]}] {type(error).__name__}")
