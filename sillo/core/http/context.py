...
async def empty_receive() -> typing.NoReturn:
    ...
    :class:`HttpContext` without a real ASGI receive channel.  This ensures
    ...
    """
    raise RuntimeError("Cannot receive. No receive channel has been made available.")
...
async def empty_send(message: Message) -> typing.NoReturn:
    ...
    :class:`HttpContext` without a real ASGI send channel.  This ensures
    ...
    """
    raise RuntimeError("Cannot send. No send channel has been made available.")
...
class HttpContext(BaseContext):
    ...
    The HttpContext object encapsulates all information about an incoming HTTP request,
    ...
    Examples:
        ...
        async def upload_file(ctx: HttpContext):
            ...
            return json({"uploaded": uploaded_file.filename})
        ```
        ...
        async def contact_form(ctx: HttpContext):
            ...
            return json({"message": "Contact form submitted"})
        ```
    """
    ...
    def __init__(
        ...
        """Initialize a HttpContext from ASGI scope and receive/send callables.
        ...
        # only be read once off the wire and more than one ``HttpContext`` gets
        ...
        while not self._stream_consumed:
            ...
            if message["type"] == "http.request":
                ...
                if not message.get("more_body", False):
                    ...
                    # Recorded on the scope so a later ``HttpContext`` built for
    ...
    def __str__(self) -> str:
        ...
        Formats the request as ``"<HttpContext METHOD URL>"`` for debugging
        and logging purposes.  The URL includes the full path and query
        string.

        Returns:
            A string in the format ``"<HttpContext GET /path?query>"`` showing
            the HTTP method and full URL.

        Raises:
            None: This method does not raise exceptions.
        """
        return f"<HttpContext {self.method} {self.url}>"
