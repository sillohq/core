    ...
    def build_routes(self) -> list:
        ...
        for model_cls, admin_cls in self.site.registry:
            ...
            routes += [
                Route(
                    b,
                    self._view(ListView, model_cls, admin_cls),
                    methods=["GET"],
                    name=f"admin-{name}-list",
                ),
                Route(
                    f"{b}create/",
                    self._view(CreateView, model_cls, admin_cls),
                    methods=["GET", "POST"],
                    name=f"admin-{name}-create",
                ),
                Route(
                    f"{b}{{id}}/",
                    self._view(DetailView, model_cls, admin_cls),
                    methods=["GET"],
                    name=f"admin-{name}-detail",
                ),
                Route(
                    f"{b}{{id}}/update/",
                    self._view(UpdateView, model_cls, admin_cls),
                    methods=["GET", "POST"],
