...
class BaseView:
    ...
    def base_ctx(self, request):
        return {
            ...
            "user_email": getattr(request, "session", {}).get("admin_user", {}).get("email", "Admin"),
    ...
    async def handle(self, request, response):
        ...
        for m in self.site.registry.models:
            ...
            try:
                ...
                pass
            dashboard_models.append({"name": m.__name__, "slug": m.__name__.lower(), "count": count})
        ctx["dashboard_models"] = dashboard_models
        ctx["recent_activity"] = [{"user_email": a.user_email, "action": a.action, "model_name": a.model_name, "created_at": str(a.created_at)[:19]} for a in recent] if recent else []
...
class ListView(BaseView):
    async def handle(self, request, response):
        ...
        for item in items:
            ...
            for ci in column_info:
                ...
                if ci["type"] == "fk" and ci["field_obj"]:
                    label, link = await _resolve_fk_value(item, ci["name"], ci["field_obj"], self.site)
                    raw = {"label": label, "link": link}
                elif ci["type"] == "m2m" and ci["field_obj"]:
                    related = await _resolve_m2m_value(item, ci["name"], ci["field_obj"], self.site)
        ...
        ctx.update({
            ...
            "sort": sort, "dir": d, "query": query,
            "columns": columns,
            "column_info": column_info,
            "rows": rows,
            "page_range": list(range(max(1, page - 2), min(total_pages, page + 2) + 1)) if total_pages > 1 else [],
        })
...
class DetailView(BaseView):
    async def handle(self, request, response, id):
        ...
        fields = []
        display_cols = [c for c in self.admin_class.list_display if not _should_skip_field(c)]
        for f in display_cols:
            ...
            if kind in ("fk", "o2o"):
                ...
                fields.append({"label": label, "value": str(getattr(obj, f, "")), "link": None, "type": "text"})
        ctx["fields"] = fields
...
class CreateView(BaseView):
    async def handle(self, request, response):
        ...
        if meta:
            for f_name in meta.fields_map:
                ...
                if _is_relation(field_obj):
                    ...
                    fields.append({
                        "name": f_name, "label": _field_label(f_name), "type": ftype,
                        "value": "", "kind": "scalar",
        ...
        if request.method == "POST":
            ...
            try:
                ...
                return response.redirect(f"{self.site.prefix}/{self.model_slug}/{obj.pk}/", status_code=302)
    ...
    async def _build_rel_field(self, field_obj, f_name, current_value):
        ...
        if kind in ("fk", "o2o"):
            rel_name, rel_slug, options = await _get_fk_options(field_obj, current_value)
            return {"name": f_name, "label": label, "kind": kind, "rel_name": rel_name, "rel_slug": rel_slug, "options": options, "value": str(current_value or ""), "required": not getattr(field_obj, "null", True)}
        if kind == "m2m":
            rel_name, rel_slug, options = await _get_m2m_options(field_obj, current_value)
            return {"name": f_name, "label": label, "kind": "m2m", "rel_name": rel_name, "rel_slug": rel_slug, "options": options, "value": []}
        return None
...
class UpdateView(BaseView):
    async def handle(self, request, response, id):
        ...
        if meta:
            for f_name in meta.fields_map:
                ...
                if _is_relation(field_obj):
                    ...
                    current_pk = getattr(current, "pk", getattr(current, "id", None)) if current and not isinstance(current, (list, type(None))) else None
        ...
        if request.method == "POST":
            ...
            try:
                for k, v in data.items():
                    if k not in meta.fields_map or k in self.admin_class.readonly_fields:
                        continue
                    field_obj = meta.fields_map[k]
                    kind = _field_kind(field_obj)
                    if kind in ("fk", "o2o"):
                        ...
                        setattr(obj, k, v)
                await obj.save()
                return response.redirect(f"{self.site.prefix}/{self.model_slug}/{id}/", status_code=302)
            except Exception as e:
                ctx["error"] = str(e)
                for fld in fields:
...
class DeleteView(BaseView):
    async def handle(self, request, response, id):
        ...
        if request.method == "POST":
            await obj.delete()
            return response.redirect(f"{self.site.prefix}/{self.model_slug}/", status_code=302)
        return response.html(_render("delete.html", **ctx))


class LoginView(BaseView):
    async def handle(self, request, response):
        ctx = {"site_title": self.site.title, "site_prefix": self.site.prefix, "error": ""}
        if request.method == "POST":
            data = dict(await request.form)
            ok = await self.site.auth.login(request, data.get("email", ""), data.get("password", ""))
            if ok:
