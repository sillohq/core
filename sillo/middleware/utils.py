...
def use_for_route(route: str) -> Callable[[HandlerType], HandlerType]:
    ...
    named ``__call__`` or ``dispatch``, it wraps it as a method with ``self`` as
    the first parameter; otherwise, it wraps it as a standalone handler function.
    ...
    def decorator(func: HandlerType) -> Any:
        ...
        async def wrapper_func(
            ...
            route pattern. If it matches, the middleware function is executed
            with the context and call_next arguments. If it does not match, the
            request is passed directly to the next handler in the chain by
            invoking ``call_next`` without executing the middleware logic.

            Args:
                ctx (HttpContext): The context for the request being handled.
        ...
        async def wrapper_klass(
            ...
            """
            if re.match(route, ctx.url.path):
                return await func(self, ctx, call_next)
            return await call_next()

        if func.__name__ in ("__call__", "dispatch"):  # ty: ignore
