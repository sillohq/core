...
from sillo.auth.session_auth.backend import login as _session_login, logout as _session_logout
...
class SessionGuard:
    ...
    async def attempt(self, request: Request, **credentials) -> bool:
        ...
        if not email or not password or self.user_model is None:
            return False
        user = await self.user_model.objects.get_by_email(email) if hasattr(self.user_model, 'objects') else None
    ...
    async def user(self, request: Request):
        ...
        if session_user and self.user_model:
            ...
            if uid:
                return await self.user_model.objects.get_by_id(int(uid)) if hasattr(self.user_model, 'objects') else None
        return None
